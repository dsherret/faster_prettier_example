...
namespace ts.codefix {
  ...
  function addMissingMembers(
    ...
    const abstractAndNonPrivateExtendsSymbols = checker
      .getPropertiesOfType(instantiatedExtendsType)
      .filter(symbolPointsToNonPrivateAndAbstractMember);
  ...
  function symbolPointsToNonPrivateAndAbstractMember(symbol: Symbol): boolean {
    ...
    return (
      !(flags & ModifierFlags.Private) && !!(flags & ModifierFlags.Abstract)
    );
  }
}
