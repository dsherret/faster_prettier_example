...
namespace ts.refactor {
  ...
  function getInfo(
    ...
    const exportingModuleSymbol = isSourceFile(exportNode.parent)
    ...
    switch (exportNode.kind) {
      ...
      case SyntaxKind.VariableStatement: {
        ...
        return (
          noSymbolError(decl.name) || {
            exportNode: vs,
            exportName: decl.name as Identifier,
            wasDefault,
            exportingModuleSymbol,
          }
        );
      ...
      case SyntaxKind.ExportAssignment: {
        ...
        return (
          noSymbolError(node.expression) || {
            exportNode: node,
            exportName: node.expression as Identifier,
            wasDefault,
            exportingModuleSymbol,
          }
        );
