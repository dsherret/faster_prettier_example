...
namespace ts.refactor {
  ...
  registerRefactor(refactorName, {
    ...
    getAvailableActions: function getRefactorActionsToMoveToNewFile(
      context
    ): readonly ApplicableRefactorInfo[] {
      const statements = getStatementsToMove(context);
      if (context.preferences.allowTextChangesInNewFiles && statements) {
        return [
          { name: refactorName, description, actions: [moveToNewFileAction] },
        ];
      }
    ...
    getEditsForAction: function getRefactorEditsToMoveToNewFile(
      context,
      actionName
    ): RefactorEditInfo {
  ...
  function getRangeToMove(context: RefactorContext): RangeToMove | undefined {
    ...
    if (
      isNamedDeclaration(startStatement) &&
      startStatement.name &&
      rangeContainsRange(startStatement.name, range)
    ) {
      return {
        toMove: [statements[startNodeIndex]],
        afterLast: statements[startNodeIndex + 1],
      };
    }
    ...
    const afterEndNodeIndex = findIndex(
      statements,
      (s) => s.end > range.end,
      startNodeIndex
    );
    ...
    if (
      afterEndNodeIndex !== -1 &&
      (afterEndNodeIndex === 0 ||
        statements[afterEndNodeIndex].getStart(file) < range.end)
    )
      return undefined;

    return {
      toMove: statements.slice(
        startNodeIndex,
        afterEndNodeIndex === -1 ? statements.length : afterEndNodeIndex
      ),
      afterLast:
        afterEndNodeIndex === -1 ? undefined : statements[afterEndNodeIndex],
    };
  }
  ...
  function doChange(
    ...
    const newModuleName = makeUniqueModuleName(
      getNewModuleName(usage.movedSymbols),
      extension,
      currentDirectory,
      host
    );
    ...
    changes.createNewFile(
      oldFile,
      combinePaths(currentDirectory, newFileNameWithExtension),
  ...
  function isPureImport(node: Node): boolean {
    switch (node.kind) {
      ...
      case SyntaxKind.VariableStatement:
        return (node as VariableStatement).declarationList.declarations.every(
          (d) =>
            !!d.initializer &&
            isRequireCall(
              d.initializer,
  ...
  function addNewFileToTsconfig(
    ...
    const newFileAbsolutePath = normalizePath(
      combinePaths(oldFileName, "..", newFileNameWithExtension)
    );
    ...
    const cfgObject =
      cfg.statements[0] &&
      tryCast(cfg.statements[0].expression, isObjectLiteralExpression);
    const filesProp =
      cfgObject &&
      find(
  ...
  function getNewStatementsAndRemoveFromOldFile(
    ...
    const checker = program.getTypeChecker();
    const prologueDirectives = takeWhile(
      oldFile.statements,
      isPrologueDirective
    );
    ...
    const importsFromNewFile = createOldFileImportsFromNewFile(
      usage.oldFileImportsFromNewFile,
      newModuleName,
      useEsModuleSyntax,
      quotePreference
    );
    ...
    deleteUnusedOldImports(
      ...
      toMove.all,
      changes,
      usage.unusedImportsFromOldFile,
      checker
    );
    deleteMovedStatements(oldFile, toMove.ranges, changes);
    ...
    const imports = getNewFileImportsAndAddExportInOldFile(
      oldFile,
      usage.oldImportsNeededByNewFile,
      usage.newFileImportsFromOldFile,
      changes,
      checker,
      useEsModuleSyntax,
      quotePreference
    );
    const body = addExports(
      oldFile,
      toMove.all,
      usage.oldFileImportsFromNewFile,
      useEsModuleSyntax
  ...
  function deleteMovedStatements(
  ...
  function deleteUnusedOldImports(
    ...
    changes: textChanges.ChangeTracker,
    toDelete: ReadonlySymbolSet,
    checker: TypeChecker
  ) {
    for (const statement of oldFile.statements) {
      if (contains(toMove, statement)) continue;
      forEachImportInStatement(statement, (i) =>
        deleteUnusedImports(oldFile, i, changes, (name) =>
          toDelete.has(checker.getSymbolAtLocation(name)!)
        )
      );
    }
  }
        ...
        forEachImportInStatement(statement, (importNode) => {
          if (
            checker.getSymbolAtLocation(
              moduleSpecifierFromImport(importNode)
            ) !== oldFile.symbol
          )
            return;
          ...
          const newModuleSpecifier = combinePaths(
            getDirectoryPath(moduleSpecifierFromImport(importNode).text),
            newModuleName
          );
  ...
  function getNamespaceLikeImport(
    ...
    switch (node.kind) {
      ...
      default:
        return Debug.assertNever(
          node,
          `Unexpected node kind ${(node as SupportedImport).kind}`
        );
    }
  }
  ...
  function updateNamespaceLikeImport(
    ...
    const preferredNewNamespaceName = codefix.moduleSpecifierToValidIdentifier(
      newModuleName,
      ScriptTarget.ESNext
    );
    ...
    if (toChange.length) {
      const newNamespaceName = needUniqueName
        ? getUniqueName(preferredNewNamespaceName, sourceFile)
        : preferredNewNamespaceName;
      for (const ref of toChange) {
        changes.replaceNode(
          sourceFile,
          ref,
          factory.createIdentifier(newNamespaceName)
        );
      }
      changes.insertNodeAfter(
        sourceFile,
  ...
  function moduleSpecifierFromImport(i: SupportedImport): StringLiteralLike {
    return i.kind === SyntaxKind.ImportDeclaration
      ? i.moduleSpecifier
      : i.kind === SyntaxKind.ImportEqualsDeclaration
      ? i.moduleReference.expression
      : i.initializer.arguments[0];
  }

  function forEachImportInStatement(
    statement: Statement,
    cb: (importNode: SupportedImport) => void
  ): void {
    ...
    if (useEs6Imports) {
      ...
      return bindingElements.length
        ? (makeVariableStatement(
            ...
            /*type*/ undefined,
            createRequireCall(factory.createStringLiteral(path))
  ...
  function addExports(
    ...
    return flatMap(toMove, (statement) => {
      if (
        ...
        forEachTopLevelDeclaration(statement, (d) =>
          needExport.has(Debug.checkDefined(d.symbol))
        )
      ) {
  ...
  function deleteUnusedImports(
    ...
    switch (importDecl.kind) {
      case SyntaxKind.ImportDeclaration:
        deleteUnusedImportsInDeclaration(
          sourceFile,
          importDecl,
          changes,
          isUnused
      ...
      default:
        Debug.assertNever(
          importDecl,
          `Unexpected import decl kind ${(importDecl as SupportedImport).kind}`
        );
    }
  }
  function deleteUnusedImportsInDeclaration(
    ...
    const namedBindingsUnused =
      !namedBindings ||
      (namedBindings.kind === SyntaxKind.NamespaceImport
        ? isUnused(namedBindings.name)
        : namedBindings.elements.length !== 0 &&
          namedBindings.elements.every((e) => isUnused(e.name)));
  ...
  function deleteUnusedImportsInVariableDeclaration(
    ...
    switch (name.kind) {
      ...
      case SyntaxKind.ObjectBindingPattern:
        if (
          name.elements.every((e) => isIdentifier(e.name) && isUnused(e.name))
        ) {
          changes.delete(
            sourceFile,
            isVariableDeclarationList(varDecl.parent) &&
              varDecl.parent.declarations.length === 1
  ...
  function getNewFileImportsAndAddExportInOldFile(
    ...
    for (const oldStatement of oldFile.statements) {
      forEachImportInStatement(oldStatement, (i) => {
        append(
          copiedOldImports,
          filterImport(i, moduleSpecifierFromImport(i), (name) =>
            importsToCopy.has(checker.getSymbolAtLocation(name)!)
          )
        );
      });
    }
    ...
    append(
      ...
      makeImportOrRequire(
        oldFileDefault,
        oldFileNamedImports,
        removeFileExtension(getBaseFileName(oldFile.fileName)),
        useEsModuleSyntax,
        quotePreference
      )
    );
  ...
  function getUsageInfo(
    ...
    const containsJsx = find(
      toMove,
      (statement) => !!(statement.transformFlags & TransformFlags.ContainsJsx)
    );
    const jsxNamespaceSymbol = getJsxNamespaceSymbol(containsJsx);
    if (jsxNamespaceSymbol) {
      // Might not exist (e.g. in non-compiling code)
    ...
    for (const statement of toMove) {
      forEachTopLevelDeclaration(statement, (decl) => {
        movedSymbols.add(
          Debug.checkDefined(
            isExpressionStatement(decl)
              ? checker.getSymbolAtLocation(decl.expression.left)
              : decl.symbol,
            "Need a symbol here"
          )
        );
      ...
      forEachReference(statement, checker, (symbol) => {
        ...
        for (const decl of symbol.declarations) {
          if (isInImport(decl)) {
            ...
            isTopLevelDeclaration(decl) &&
            sourceFileOfTopLevelDeclaration(decl) === oldFile &&
    ...
    for (const statement of oldFile.statements) {
      ...
      if (
        jsxNamespaceSymbol &&
        !!(statement.transformFlags & TransformFlags.ContainsJsx)
      ) {
    ...
    function getJsxNamespaceSymbol(containsJsx: Node | undefined) {
      ...
      return !!jsxNamespaceSymbol &&
        some(jsxNamespaceSymbol.declarations, isInImport)
        ? jsxNamespaceSymbol
        : undefined;
    }
  }
  ...
  function isInImport(decl: Declaration) {
    switch (decl.kind) {
      ...
      case SyntaxKind.BindingElement:
        return (
          isVariableDeclaration(decl.parent.parent) &&
          isVariableDeclarationInImport(decl.parent.parent)
        );
      default:
        return false;
    }
  ...
  function filterImport(
    ...
    switch (i.kind) {
      case SyntaxKind.ImportDeclaration: {
        ...
        const defaultImport =
          clause.name && keep(clause.name) ? clause.name : undefined;
        const namedBindings =
          clause.namedBindings &&
          filterNamedBindings(clause.namedBindings, keep);
      ...
      case SyntaxKind.VariableDeclaration: {
        ...
        return name
          ? makeVariableStatement(
              name,
              i.type,
              createRequireCall(moduleSpecifier),
              i.parent.flags
            )
          : undefined;
      }
      default:
        return Debug.assertNever(
          i,
          `Unexpected import kind ${(i as SupportedImport).kind}`
        );
    }
  }
  ...
  function filterBindingName(
    ...
    switch (name.kind) {
      ...
      case SyntaxKind.ObjectBindingPattern: {
        ...
        const newElements = name.elements.filter(
          (prop) =>
            prop.propertyName || !isIdentifier(prop.name) || keep(prop.name)
        );
        return newElements.length
          ? factory.createObjectBindingPattern(newElements)
          : undefined;
      }
    }
  }
  ...
  type NonVariableTopLevelDeclaration =
    ...
    | ImportEqualsDeclaration;
  type TopLevelDeclarationStatement =
  ...
  function isTopLevelDeclaration(node: Node): node is TopLevelDeclaration {
    return (
      (isNonVariableTopLevelDeclaration(node) && isSourceFile(node.parent)) ||
  ...
  function isTopLevelDeclarationStatement(
    node: Node
  ): node is TopLevelDeclarationStatement {
    Debug.assert(
      isSourceFile(node.parent),
      "Node parent should be a SourceFile"
    );
  ...
  function isNonVariableTopLevelDeclaration(
    node: Node
  ): node is NonVariableTopLevelDeclaration {
    switch (node.kind) {
  ...
  function forEachTopLevelDeclaration<T>(
    statement: Statement,
  ...
  function forEachTopLevelDeclarationInBindingName<T>(
    ...
    switch (name.kind) {
      ...
      case SyntaxKind.ArrayBindingPattern:
      case SyntaxKind.ObjectBindingPattern:
        return firstDefined(name.elements, (em) =>
          isOmittedExpression(em)
            ? undefined
            : forEachTopLevelDeclarationInBindingName(em.name, cb)
        );
      default:
        return Debug.assertNever(
          name,
          `Unexpected name kind ${(name as BindingName).kind}`
        );
    }
  }

  function nameOfTopLevelDeclaration(
    d: TopLevelDeclaration
  ): Identifier | undefined {
    return isExpressionStatement(d)
      ? tryCast(d.expression.left.name, isIdentifier)
      : tryCast(d.name, isIdentifier);
  }
    ...
    if (useEs6Exports) {
      ...
      if (names.length !== 0)
        changes.insertNodesAfter(
          sourceFile,
          decl,
          names.map(createExportAssignment)
        );
    }
  }
  ...
  function isExported(
    ...
    if (useEs6Exports) {
      return (
        ...
        !!(name && sourceFile.symbol.exports?.has(name.escapedText))
      );
    }
    return getNamesToExportInCommonJS(decl).some((name) =>
  ...
  function addEs6Export(
    d: TopLevelDeclarationStatement
  ): TopLevelDeclarationStatement {
    const modifiers = concatenate(
    ...
    switch (d.kind) {
      ...
      default:
        return Debug.assertNever(
          d,
          `Unexpected declaration kind ${(d as DeclarationStatement).kind}`
        );
    }
  }
  ...
  function getNamesToExportInCommonJS(
    ...
    switch (decl.kind) {
      ...
      case SyntaxKind.VariableStatement:
        return mapDefined(decl.declarationList.declarations, (d) =>
          isIdentifier(d.name) ? d.name.text : undefined
        );
      case SyntaxKind.ModuleDeclaration:
      case SyntaxKind.EnumDeclaration:
      case SyntaxKind.TypeAliasDeclaration:
      ...
      default:
        return Debug.assertNever(
          decl,
          `Unexpected decl kind ${(decl as TopLevelDeclarationStatement).kind}`
        );
    }
  }
