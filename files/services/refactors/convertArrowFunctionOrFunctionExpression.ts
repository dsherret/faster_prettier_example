...
namespace ts.refactor.convertArrowFunctionOrFunctionExpression {
  ...
  const refactorDescription = getLocaleSpecificMessage(
    Diagnostics.Convert_arrow_function_or_function_expression
  );

  const toAnonymousFunctionAction = {
    name: "Convert to anonymous function",
    description: getLocaleSpecificMessage(
  ...
  const toNamedFunctionAction = {
    ...
    description: getLocaleSpecificMessage(
      Diagnostics.Convert_to_named_function
    ),
  ...
  const toArrowFunctionAction = {
    ...
    description: getLocaleSpecificMessage(
      Diagnostics.Convert_to_arrow_function
    ),
  ...
  function getRefactorActionsToConvertFunctionExpressions(
    ...
    if (refactorKindBeginsWith(toNamedFunctionAction.kind, kind)) {
      const error =
        ...
        (isArrowFunction(func) && isVariableDeclaration(func.parent))
          ? undefined
          : getLocaleSpecificMessage(
              Diagnostics.Could_not_convert_to_named_function
            );
      if (error) {
    ...
    if (refactorKindBeginsWith(toAnonymousFunctionAction.kind, kind)) {
      const error =
        !selectedVariableDeclaration && isArrowFunction(func)
          ? undefined
          : getLocaleSpecificMessage(
              Diagnostics.Could_not_convert_to_anonymous_function
            );
      if (error) {
        errors.push({
    ...
    if (refactorKindBeginsWith(toArrowFunctionAction.kind, kind)) {
      const error = isFunctionExpression(func)
        ? undefined
        : getLocaleSpecificMessage(
            Diagnostics.Could_not_convert_to_arrow_function
          );
      if (error) {
  ...
  function getRefactorEditsToConvertFunctionExpressions(
    ...
    switch (actionName) {
      ...
      case toNamedFunctionAction.name:
        ...
        edits.push(
          ...getEditInfoForConvertToNamedFunction(context, func, variableInfo)
        );
        break;
  ...
  function getFunctionInfo(
    ...
    const func = tryGetFunctionFromVariableDeclaration(
      file,
      typeChecker,
      token.parent
    );
    if (
      func &&
      !containingThis(func.body) &&
      !typeChecker.containsArgumentsReference(func)
    ) {
    ...
    if (
      ...
      if (
        ...
        isFunctionReferencedInFile(file, typeChecker, maybeFunc)
      )
        return undefined;
  ...
  function tryGetFunctionFromVariableDeclaration(
    ...
    const variableDeclaration = isVariableDeclaration(parent)
      ? parent
      : first(parent.declarations);
    const initializer = variableDeclaration.initializer;
    if (
      initializer &&
      (isArrowFunction(initializer) ||
        (isFunctionExpression(initializer) &&
          !isFunctionReferencedInFile(sourceFile, typeChecker, initializer)))
    ) {
      return initializer;
    }
    return undefined;
  ...
  function getVariableInfo(
    ...
    if (
      ...
      !isVariableDeclarationInVariableStatement(variableDeclaration)
    )
      return undefined;
    ...
    if (
      !isVariableDeclarationList(variableDeclarationList) ||
  ...
  function getEditInfoForConvertToAnonymousFunction(
    ...
    return textChanges.ChangeTracker.with(context, (t) =>
      t.replaceNode(file, func, newNode)
    );
  }
  ...
  function getEditInfoForConvertToNamedFunction(
    ...
    const { variableDeclaration, variableDeclarationList, statement, name } =
      variableInfo;
    suppressLeadingTrivia(statement);

    const modifiersFlags =
      (getCombinedModifierFlags(variableDeclaration) & ModifierFlags.Export) |
    ...
    if (variableDeclarationList.declarations.length === 1) {
      return textChanges.ChangeTracker.with(context, (t) =>
        t.replaceNode(file, statement, newNode)
      );
    } else {
      return textChanges.ChangeTracker.with(context, (t) => {
  ...
  function getEditInfoForConvertToArrowFunction(
    context: RefactorContext,
  ...
  function canBeConvertedToExpression(
    body: Block,
    head: Statement
  ): head is ReturnStatement {
    return (
      body.statements.length === 1 &&
      isReturnStatement(head) &&
      !!head.expression
    );
  }
