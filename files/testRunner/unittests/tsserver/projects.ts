namespace ts.projectSystem {
  describe("unittests:: tsserver:: Projects", () => {
    it("handles the missing files - that were added to program because they were added with ///<ref", () => {
      ...
      const getErrRequest =
        makeSessionRequest<server.protocol.SemanticDiagnosticsSyncRequestArgs>(
      ...
      baselineTsserverLogs(
        "projects",
        "handles the missing files added with tripleslash ref",
        session
      );
    });
    ...
    describe("ignoreConfigFiles", () => {
      ...
      it("loose file included in config file (applyCodeChanges)", () => {
        ...
        projectService.applyChangesInOpenFiles(
          singleIterator({ fileName: file1.path, content: file1.content })
        );
    ...
    it("reload regular file after closing", () => {
      ...
      checkProjectActualFiles(service.externalProjects[0], [
        f1.path,
        f2.path,
        libFile.path,
      ]);

      const completions1 = service.externalProjects[0]
        .getLanguageService()
        .getCompletionsAtPosition(f1.path, 2, emptyOptions)!;
      // should contain completions for string
      assert.isTrue(
        completions1.entries.some((e) => e.name === "charAt"),
        "should contain 'charAt'"
      );
      assert.isFalse(
        completions1.entries.some((e) => e.name === "toExponential"),
        "should not contain 'toExponential'"
      );

      service.closeClientFile(f2.path);
      const completions2 = service.externalProjects[0]
        .getLanguageService()
        .getCompletionsAtPosition(f1.path, 2, emptyOptions)!;
      // should contain completions for string
      assert.isFalse(
        completions2.entries.some((e) => e.name === "charAt"),
        "should not contain 'charAt'"
      );
      assert.isTrue(
        completions2.entries.some((e) => e.name === "toExponential"),
        "should contain 'toExponential'"
      );
    });
    ...
    it("clear mixed content file after closing", () => {
      ...
      service.openExternalProject({
        ...
        rootFiles: [
          { fileName: f1.path },
          { fileName: f2.path, hasMixedContent: true },
        ],
        options: {},
      });
      ...
      const completions1 = service.externalProjects[0]
        .getLanguageService()
        .getCompletionsAtPosition(f1.path, 0, emptyOptions)!;
      assert.isTrue(
        completions1.entries.some((e) => e.name === "somelongname"),
        "should contain 'somelongname'"
      );
      ...
      const completions2 = service.externalProjects[0]
        ...
        .getCompletionsAtPosition(f1.path, 0, emptyOptions)!;
      assert.isFalse(
        completions2.entries.some((e) => e.name === "somelongname"),
        "should not contain 'somelongname'"
      );
      const sf2 = service.externalProjects[0]
        .getLanguageService()
        .getProgram()!
        .getSourceFile(f2.path)!;
    ...
    it("ignores files excluded by a custom safe type list", () => {
      ...
      try {
        ...
        const proj = projectService.externalProjects[0];
        assert.deepEqual(
          proj.getFileNames(/*excludeFilesFromExternalLibraries*/ true),
          [file1.path]
        );
    ...
    it("file with name constructor.js doesnt cause issue with typeAcquisition when safe type list", () => {
      ...
      const typingsInstaller: server.ITypingsInstaller = {
        ...
        enqueueInstallTypingsRequest: (
          ...
          request = JSON.stringify(
            server.createInstallTypingsRequest(
              proj,
              typeAcquisition,
      ...
      projectService.openExternalProject({
        ...
        options: {},
        rootFiles: toExternalFiles([
          file1.path,
          constructorFile.path,
          bliss.path,
        ]),
      });
      assert.equal(
    ...
    it("ignores files excluded by the default type list", () => {
      ...
      try {
        ...
        assert.deepEqual(proj.getTypeAcquisition().include, [
          "kendo-ui",
          "office",
        ]);
      } finally {
    ...
    it("correctly migrate files between projects", () => {
      ...
      assert.strictEqual(
        projectService.inferredProjects[2],
        inferredProjects[2]
      );
      assert.isTrue(projectService.inferredProjects[0].isOrphan());
      checkProjectActualFiles(projectService.inferredProjects[1], [file2.path]);
    ...
    it("regression test for crash in acquireOrUpdateDocument", () => {
      ...
      projectService
        .findProject(projs[0].info!.projectName)!
        .getLanguageService()
        .getNavigationBarItems(tsFile.fileName);
    ...
    it("config file is deleted", () => {
      ...
      checkProjectActualFiles(configuredProjectAt(projectService, 0), [
        file1.path,
        file2.path,
        config.path,
      ]);
    ...
    it("tsconfig script block support", () => {
      ...
      const configureHostRequest =
        makeSessionRequest<protocol.ConfigureRequestArguments>(
      ...
      assert.strictEqual(
        ...
        configuredProj,
        "Same configured project should be updated"
      );
      checkProjectActualFiles(configuredProjectAt(projectService, 0), [
        file1.path,
        file2.path,
      ...
      let completions = project
        .getLanguageService()
        .getCompletionsAtPosition(file1.path, 1, emptyOptions);
      assert(
        completions && some(completions.entries, (e) => e.name === "hello"),
        `expected entry hello to be in completion list`
      );
      ...
      completions = project
        .getLanguageService()
        .getCompletionsAtPosition(file1.path, 5, emptyOptions);
      assert(
        completions && completions.entries[0].name !== "hello",
        `unexpected hello entry in completion list`
      );
    });
    ...
    it("no tsconfig script block diagnostic errors", () => {
      ...
      let host = createServerHost([file1, file2, config1, libFile], {
        executingFilePath: combinePaths(
          getDirectoryPath(libFile.path),
          "tsc.js"
        ),
      });
      ...
      let diagnostics = configuredProjectAt(projectService, 0)
        .getLanguageService()
      ...
      host = createServerHost([file1, file2, config2, libFile], {
        executingFilePath: combinePaths(
          getDirectoryPath(libFile.path),
          "tsc.js"
        ),
      ...
      diagnostics = configuredProjectAt(projectService, 0)
        .getLanguageService()
        .getCompilerOptionsDiagnostics();
      assert.deepEqual(diagnostics, []);
      ...
      host = createServerHost([file1, file2, config3, libFile], {
        executingFilePath: combinePaths(
          getDirectoryPath(libFile.path),
          "tsc.js"
        ),
      ...
      host = createServerHost([file1, file2, config4, libFile], {
        executingFilePath: combinePaths(
          getDirectoryPath(libFile.path),
          "tsc.js"
        ),
      ...
      host = createServerHost([file1, file2, config5, libFile], {
        executingFilePath: combinePaths(
          getDirectoryPath(libFile.path),
          "tsc.js"
        ),
    ...
    it("project structure update is deferred if files are not added\removed", () => {
      ...
      projectService.applyChangesInOpenFiles(
        /*openFiles*/ undefined,
        /*changedFiles*/ singleIterator({
          fileName: file1.path,
          changes: singleIterator({
            span: createTextSpan(0, file1.path.length),
            newText: "let y = 1",
          }),
        }),
    ...
    it("files with mixed content are handled correctly", () => {
      ...
      assert.equal(
        actualText,
        "",
        `expected content to be empty string, got "${actualText}"`
      );
      ...
      const quickInfo = project
        .getLanguageService()
        .getQuickInfoAtPosition(file1.path, 4)!;
      ...
      assert.equal(
        actualText2,
        "",
        `expected content to be empty string, got "${actualText2}"`
      );
    });
    ...
    it("File in multiple projects at opened and closed correctly", () => {
      ...
      assert.isTrue(
        project1.hasOpenRef(),
        "Has open ref count in project1 - 1"
      ); // file2
      assert.equal(
        project1.getScriptInfo(file2.path)!.containingProjects.length,
        1,
        "containing projects count"
      );
      ...
      assert.isTrue(
        project1.hasOpenRef(),
        "Has open ref count in project1 - 2"
      ); // file2
      assert.strictEqual(
        projectService.configuredProjects.get(tsconfig1.path),
        project1
      );
      assert.isFalse(project1.isClosed());
      ...
      assert.isTrue(
        project2.hasOpenRef(),
        "Has open ref count in project2 - 2"
      ); // file1
      assert.isFalse(project2.isClosed());

      assert.equal(
        project1.getScriptInfo(file1.path)!.containingProjects.length,
        2,
        `${file1.path} containing projects count`
      );
      assert.equal(
        project1.getScriptInfo(file2.path)!.containingProjects.length,
        1,
        `${file2.path} containing projects count`
      );
      ...
      assert.isFalse(
        project1.hasOpenRef(),
        "Has open ref count in project1 - 3"
      ); // No files
      assert.isTrue(
        project2.hasOpenRef(),
        "Has open ref count in project2 - 3"
      ); // file1
      assert.strictEqual(
        projectService.configuredProjects.get(tsconfig1.path),
        project1
      ...
      assert.strictEqual(
        projectService.configuredProjects.get(tsconfig2.path),
        project2
      );
      assert.isFalse(project1.isClosed());
      assert.isFalse(project2.isClosed());
      ...
      assert.isFalse(
        project1.hasOpenRef(),
        "Has open ref count in project1 - 4"
      ); // No files
      assert.isFalse(
        project2.hasOpenRef(),
        "Has open ref count in project2 - 4"
      ); // No files
      assert.strictEqual(
        projectService.configuredProjects.get(tsconfig1.path),
        project1
      ...
      assert.isTrue(
        project1.hasOpenRef(),
        "Has open ref count in project1 - 5"
      ); // file2
      assert.isFalse(project1.isClosed());
    ...
    it("snapshot from different caches are incompatible", () => {
      ...
      projectService.externalProjects[0]
        .getLanguageService(/*ensureSynchronized*/ false)
        .getNavigationBarItems(f1.path);
      projectService.closeClientFile(f1.path);

      projectService.openClientFile(f1.path);
      projectService.checkNumberOfProjects({ externalProjects: 1 });
      const navbar = projectService.externalProjects[0]
        .getLanguageService(/*ensureSynchronized*/ false)
        .getNavigationBarItems(f1.path);
    ...
    it("Properly handle Windows-style outDir", () => {
      ...
      checkProjectActualFiles(project, [
        normalizePath(file1.path),
        normalizePath(configFile.path),
      ]);
    ...
    it("files opened, closed affecting multiple projects", () => {
      ...
      const projectService = session.getProjectService();
      const configuredProject = projectService.configuredProjects.get(
        config.path
      )!;
      ...
      function verifyConfiguredProject() {
        checkNumberOfProjects(projectService, { configuredProjects: 1 });
        checkProjectActualFiles(configuredProject, [
          file.path,
          filesFile1.path,
          libFile.path,
          config.path,
    ...
    describe("includes deferred files in the project context", () => {
      function verifyDeferredContext(
        ...
        assert.isTrue(
          configuredProject.getCompilerOptions().allowNonTsExtensions
        );
      }

      it("when lazyConfiguredProjectsFromExternalProject not set", () => {
        verifyDeferredContext(
    ...
    it("Orphan source files are handled correctly on watch trigger", () => {
      ...
      function verifyFile2InfoIsOrphan() {
        const info = Debug.checkDefined(
          service.getScriptInfoForPath(file2.path as Path)
        );
    ...
    it("no project structure update on directory watch invoke on open file save", () => {
      ...
      host.modifyFile(file1.path, file1.content, {
  ...
  "include": [
      ...
      const newKnownProjects = projectService.synchronizeProjectList(
        knownProjects.map((proj) => proj.info!),
        /*includeProjectReferenceRedirectInfo*/ true
      );
    ...
    it("synchronizeProjectList returns correct information when base configuration file cannot be resolved and redirect info is requested", () => {
      ...
      assert.deepEqual(knownProjects[0].files, [
        ...
        {
          fileName: `${tscWatch.projectRoot}/tsconfig_base.json`,
          isSourceOfProjectReferenceRedirect: false,
        },
      ]);
    });
    ...
    it("handles delayed directory watch invoke on file creation", () => {
      ...
      host.fileExists = (s) =>
        s === fileA.path ? false : originalFileExists.call(host, s);
      ...
      baselineTsserverLogs(
        "projects",
        "handles delayed directory watch invoke on file creation",
        session
      );
    ...
    it("assert when removing project", () => {
      ...
      const info = service.getOrCreateScriptInfoForNormalizedPath(
        commonFile2.path as server.NormalizedPath,
    ...
    it("does not look beyond node_modules folders for default configured projects", () => {
      ...
      const nodeModulesFilePath1 = server.asNormalizedPath(
        "/project/node_modules/@types/a/index.d.ts"
      );
      const nodeModulesProjectPath1 = server.asNormalizedPath(
        "/project/node_modules/@types/a/tsconfig.json"
      );
      const nodeModulesFilePath2 = server.asNormalizedPath(
      ...
      assert.strictEqual(
        openRootFileResult.configFileName?.toString(),
        rootProjectPath
      );

      const openNodeModulesFileResult1 =
        projectService.openClientFile(nodeModulesFilePath1);
      assert.strictEqual(
        openNodeModulesFileResult1.configFileName?.toString(),
        nodeModulesProjectPath1
      );

      const openNodeModulesFileResult2 =
        projectService.openClientFile(nodeModulesFilePath2);
      assert.isUndefined(openNodeModulesFileResult2.configFileName);

      const rootProject = projectService.findProject(rootProjectPath)!;
      checkProjectActualFiles(rootProject, [
        rootProjectPath,
        rootFilePath,
        nodeModulesFilePath1,
        nodeModulesFilePath2,
      ]);
