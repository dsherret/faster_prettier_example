namespace ts.projectSystem {
  describe("unittests:: tsserver:: rename", () => {
    it("works with fileToRename", () => {
      ...
      const response1 = executeSessionRequest<
      ...
      session.getProjectService().setHostConfiguration({
        preferences: { allowRenameOfImportPath: true },
      });
      const response2 = executeSessionRequest<
        protocol.RenameRequest,
        protocol.RenameResponse
      >(
        session,
      ...
      assert.deepEqual<protocol.RenameResponseBody | undefined>(response2, {
        info: {
          ...
          kindModifiers: "",
          triggerSpan: protocolTextSpanFromSubstring(bTs.content, "a", {
            index: 1,
          }),
        },
        locs: [
      ...
      const response3 = executeSessionRequest<
        protocol.RenameRequest,
        protocol.RenameResponse
      >(
        session,
    ...
    it("export default anonymous function works with prefixText and suffixText when disabled", () => {
      ...
      const bTs: File = {
        path: "/b.ts",
        content: `import aTest from "./a"; function test() { return aTest(); }`,
      };
      ...
      assert.deepEqual<protocol.RenameResponseBody | undefined>(response1, {
        info: {
          ...
          triggerSpan: protocolTextSpanFromSubstring(bTs.content, "aTest", {
            index: 1,
          }),
        },
        locs: [
    ...
    it("rename behavior is based on file of rename initiation", () => {
      const aTs: File = {
        path: "/a.ts",
        content: "const x = 1; export { x };",
      };
      const bTs: File = {
        path: "/b.ts",
        content: `import { x } from "./a"; const y = x + 1;`,
      };
