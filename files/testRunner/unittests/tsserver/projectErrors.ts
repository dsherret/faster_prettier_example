namespace ts.projectSystem {
  describe("unittests:: tsserver:: Project Errors", () => {
    function checkProjectErrors(
      projectFiles: server.ProjectFilesWithTSDiagnostics,
    ...
    function checkProjectErrorsWorker(
      ...
      assert.equal(
        ...
        `expected ${expectedErrors.length} error in the list`
      );
      if (expectedErrors.length) {
        for (let i = 0; i < errors.length; i++) {
          const actualMessage = flattenDiagnosticMessageText(
            errors[i].messageText,
            "\n"
          );
          const expectedMessage = expectedErrors[i];
          assert.isTrue(
            actualMessage.indexOf(expectedMessage) === 0,
            `error message does not match, expected ${actualMessage} to start with ${expectedMessage}`
          );
        }
      }
    }

    function checkDiagnosticsWithLinePos(
      errors: server.protocol.DiagnosticWithLinePosition[],
      ...
      if (expectedErrors.length) {
        zipWith(
          ...
          ({ message: actualMessage }, expectedMessage) => {
            assert.isTrue(
              startsWith(actualMessage, actualMessage),
              `error message does not match, expected ${actualMessage} to start with ${expectedMessage}`
            );
          }
        );
    ...
    it("external project - diagnostics for missing files", () => {
...

      {
        ...
        const diags = session.executeCommand(compilerOptionsRequest)
          .response as server.protocol.DiagnosticWithLinePosition[];
        // only file1 exists - expect error
        checkDiagnosticsWithLinePos(diags, [
          "File '/a/b/applib.ts' not found.",
        ]);
      }
    ...
    it("configured projects - diagnostics for missing files", () => {
      ...
      const config = {
        ...
        content: JSON.stringify({
          files: [file1, file2].map((f) => getBaseFileName(f.path)),
        }),
      };
      ...
      let diags = session.executeCommand(compilerOptionsRequest)
        .response as server.protocol.DiagnosticWithLinePosition[];
      ...
      checkNumberOfProjects(projectService, { configuredProjects: 1 });
      diags = session.executeCommand(compilerOptionsRequest)
        .response as server.protocol.DiagnosticWithLinePosition[];
      checkDiagnosticsWithLinePos(diags, []);
    });
    ...
    it("configured projects - diagnostics for corrupted config 1", () => {
      ...
      projectService.openClientFile(file1.path);
      {
        ...
        const configuredProject = find(
          projectService.synchronizeProjectList([]),
          (f) => f.info!.projectName === corruptedConfig.path
        )!;
        assert.isTrue(
          configuredProject !== undefined,
          "should find configured project"
        );
        checkProjectErrors(configuredProject, []);
        const projectErrors = configuredProjectAt(
          projectService,
          0
        ).getAllProjectErrors();
        checkProjectErrorsWorker(projectErrors, ["'{' expected."]);
...
/// <reference path="${refPathNotFound2}" />`;
    ...
    it("folder rename updates project structure and reports no errors", () => {
      ...
      baselineTsserverLogs(
        "projectErrors",
        `folder rename updates project structure and reports no errors`,
        session
      );
    });
    ...
    it("Getting errors before opening file", () => {
      ...
      baselineTsserverLogs(
        "projectErrors",
        "getting errors before opening file",
        session
      );
    });
    ...
    it("Reports errors correctly when file referenced by inferred project root, is opened right after closing the root file", () => {
      ...
      openFilesForSession(
        [{ file: app, projectRootPath: tscWatch.projectRoot }],
        session
      );
      openFilesForSession(
        [{ file: backendTest, projectRootPath: tscWatch.projectRoot }],
        session
      );
      verifyGetErrRequest({
        session,
        host,
        files: [backendTest.path, app.path],
      });
      closeFilesForSession([backendTest], session);
      openFilesForSession(
        [{ file: serverUtilities.path, projectRootPath: tscWatch.projectRoot }],
        session
      );
      verifyGetErrRequest({
        session,
        host,
        files: [serverUtilities.path, app.path],
      });
      baselineTsserverLogs(
        "projectErrors",
...
declare module '@custom/plugin' {
  ...
  describe("unittests:: tsserver:: Project Errors dont include overwrite emit error", () => {
    it("for inferred project", () => {
      ...
      } as server.protocol.CompilerOptionsDiagnosticsRequest)
  ...
  describe("unittests:: tsserver:: Project Errors reports Options Diagnostic locations correctly with changes in configFile contents", () => {
    it("when options change", () => {
      ...
      const configFileContentWithoutCommentLine =
        configFileContentBeforeComment + configFileContentAfterComment;
      ...
      const projectService = session.getProjectService();
      checkNumberOfProjects(projectService, { configuredProjects: 1 });
      const projectName = configuredProjectAt(
        projectService,
        0
      ).getProjectName();
      ...
      } as server.protocol.SemanticDiagnosticsSyncRequest)
        .response as readonly server.protocol.DiagnosticWithLinePosition[];
      ...
      function verifyDiagnostic(
        beforeEditDiag: server.protocol.DiagnosticWithLinePosition,
        afterEditDiag: server.protocol.DiagnosticWithLinePosition
      ) {
        ...
        assert.equal(
          beforeEditDiag.startLocation.line,
          afterEditDiag.startLocation.line + 1
        );
        assert.equal(
          beforeEditDiag.startLocation.offset,
          afterEditDiag.startLocation.offset
        );
        assert.equal(
          beforeEditDiag.endLocation.line,
          afterEditDiag.endLocation.line + 1
        );
        assert.equal(
          beforeEditDiag.endLocation.offset,
          afterEditDiag.endLocation.offset
        );
      }
    });
  });
  ...
  describe("unittests:: tsserver:: Project Errors with config file change", () => {
    it("Updates diagnostics when '--noUnusedLabels' changes", () => {
      ...
      const tsconfig: File = {
        path: "/tsconfig.json",
        content: options(/*allowUnusedLabels*/ true),
      };
      ...
      >(session, protocol.CommandTypes.SemanticDiagnosticsSync, {
        file: aTs.path,
      }) as protocol.Diagnostic[] | undefined;
...
console.log(blabla);`,
  ...
  describe("unittests:: tsserver:: Project Errors with npm install when", () => {
    function verifyNpmInstall(timeoutDuringPartialInstallation: boolean) {
      ...
      openFilesForSession(
        [{ file: main, projectRootPath: tscWatch.projectRoot }],
        session
      );
      ...
      filesAndFoldersToAdd = [
        ...
        {
          path: `${tscWatch.projectRoot}/node_modules/.staging/@angular/cli-c1e44b05/models/analytics.d.ts`,
          content: `export const x = 10;`,
        },
        {
          path: `${tscWatch.projectRoot}/node_modules/.staging/@angular/core-0963aebf/index.d.ts`,
          content: `export const y = 10;`,
        },
      ];
      ...
      host.deleteFolder(
        `${tscWatch.projectRoot}/node_modules/.staging`,
        /*recursive*/ true
      );
      ...
      baselineTsserverLogs(
        "projectErrors",
        `npm install when timeout occurs ${
          timeoutDuringPartialInstallation ? "inbetween" : "after"
        } installation`,
        session
      );
      ...
      function verifyWhileNpmInstall(timeouts: number) {
        ...
        verifyGetErrRequest({
          ...
          files: [main],
          existingTimeouts:
            !npmInstallComplete && !timeoutDuringPartialInstallation
              ? timeouts
                ? 3
                : 2
              : undefined,
