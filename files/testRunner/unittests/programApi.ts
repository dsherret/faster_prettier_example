namespace ts {
  function verifyMissingFilePaths(
    ...
    const notFound = arrayFrom(
      mapDefinedIterator(map.keys(), (k) => (map.has(k) ? k : undefined))
    );
    assert.equal(
      notFound.length,
      0,
      `Not found ${notFound} in actual: ${missingPaths} expected: ${expected}`
    );
  }
  ...
  describe("unittests:: programApi:: Program.getMissingFilePaths", () => {
    ...
    const testCompilerHost = new fakes.CompilerHost(
      vfs.createFromFileSystem(Harness.IO, /*ignoreCase*/ true, {
        documents: [emptyFile, referenceFile],
        cwd: "d:\\pretend\\",
      }),
    ...
    it("handles multiple missing root files", () => {
      const program = createProgram(
        ["./nonexistent0.ts", "./nonexistent1.ts"],
        options,
        testCompilerHost
      );
      const missing = program.getMissingFilePaths();
      verifyMissingFilePaths(missing, [
        "d:/pretend/nonexistent0.ts",
        "d:/pretend/nonexistent1.ts",
      ]);
    });
    ...
    it("should not have missing file paths", () => {
      ...
      const host: CompilerHost = {
        getSourceFile: (
          ...
          return fileName === "test.ts"
            ? createSourceFile(fileName, testSource, languageVersion)
            : undefined;
        },
        ...
        readFile: (fileName) =>
          fileName === "test.ts" ? testSource : undefined,
        resolveModuleNames: (
          _moduleNames: string[],
      ...
      assert(
        program.getSourceFiles().length === 1,
        "expected 'getSourceFiles' length to be 1"
      );
      assert(
        program.getMissingFilePaths().length === 0,
        "expected 'getMissingFilePaths' length to be 0"
      );
      assert(
  ...
  describe("unittests:: Program.isSourceFileFromExternalLibrary", () => {
    it("works on redirect files", () => {
      ...
      const a = new documents.TextDocument(
        "/a.ts",
        'import * as bar from "bar"; import * as foo from "foo";'
      );
      const bar = new documents.TextDocument(
      ...
      const barFooIndex = new documents.TextDocument(
        "/node_modules/bar/node_modules/foo/index.d.ts",
        fooIndexText
      );
      const fooPackage = new documents.TextDocument(
        "/node_modules/foo/package.json",
        fooPackageJsonText
      );
      const fooIndex = new documents.TextDocument(
    ...
    function assertIsExternal(
      ...
      for (const file of files) {
        const actual = program.isSourceFileFromExternalLibrary(
  ...
  describe("unittests:: Program.getNodeCount / Program.getIdentifierCount", () => {
    it("works on projects that have .json files", () => {
      const main = new documents.TextDocument(
        "/main.ts",
        'export { version } from "./package.json";'
      );
      const pkg = new documents.TextDocument(
        "/package.json",
        '{"version": "1.0.0"}'
      );
  ...
  describe("unittests:: programApi:: Program.getTypeChecker / Program.getSemanticDiagnostics", () => {
    ...
    it("getSymbolAtLocation does not cause additional error to be added on module resolution failure", () => {
      ...
      const mod = new documents.TextDocument(
        "/module.d.ts",
      ...
      typeChecker.getSymbolAtLocation(
        (sourceFile.statements[0] as ImportDeclaration).moduleSpecifier
      );
