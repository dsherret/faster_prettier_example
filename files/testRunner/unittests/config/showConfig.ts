namespace ts {
  describe("unittests:: config:: showConfig", () => {
    function showTSConfigCorrectly(
      name: string,
      commandLinesArgs: string[],
      configJson?: object
    ) {
      describe(name, () => {
        const outputFileName = `showConfig/${name.replace(
          /[^a-z0-9\-./ ]/gi,
          ""
        )}/tsconfig.json`;
        ...
        it(`Correct output for ${outputFileName}`, () => {
          ...
          const configParseHost: ParseConfigFileHost = {
            fileExists: (path) =>
              comparePaths(getNormalizedAbsolutePath(path, cwd), configPath) ===
              Comparison.EqualTo
                ? true
                : false,
            getCurrentDirectory() {
          ...
          const initResult = convertToTSConfig(
            commandLine,
            configPath,
            configParseHost
          );
          ...
          Harness.Baseline.runBaseline(
            outputFileName,
            JSON.stringify(initResult, null, 4) + "\n"
          );
        });
      });
    }
    ...
    showTSConfigCorrectly("Show TSConfig with files options", [
      "--showConfig",
      "file0.ts",
      "file1.ts",
      "file2.ts",
    ...
    showTSConfigCorrectly("Show TSConfig with boolean value compiler options", [
      "--showConfig",
      "--noUnusedLocals",
    ]);
    ...
    showTSConfigCorrectly("Show TSConfig with list compiler options", [
      "--showConfig",
      "--types",
      "jquery,mocha",
    ]);
    ...
    showTSConfigCorrectly(
      ...
      ["--showConfig", "--lib", "es5,es2015.core"]
    );

    showTSConfigCorrectly("Show TSConfig with incorrect compiler option", [
      "--showConfig",
      "--someNonExistOption",
    ]);
    ...
    showTSConfigCorrectly(
      ...
      ["--showConfig", "--lib", "nonExistLib,es5,es2015.promise"]
    );
    ...
    function baselineOption(
      ...
      const configObject =
        optionValue &&
        (isCompilerOptions
          ? { compilerOptions: optionValue }
          : { watchOptions: optionValue });
      showTSConfigCorrectly(
        `Shows tsconfig for single option/${option.name}`,
        args,
        configObject
      );
    }
