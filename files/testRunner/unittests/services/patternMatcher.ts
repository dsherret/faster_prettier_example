describe("unittests:: services:: PatternMatcher", () => {
  ...
  describe("SingleWordPattern", () => {
    ...
    it("PreferCaseSensitiveTryUnderscoredName", () => {
      assertSegmentMatch("_fogBar", "_fB", {
        kind: ts.PatternMatchKind.camelCase,
        isCaseSensitive: true,
      });
    });
    ...
    it("PreferCaseSensitiveMiddleUnderscore2", () => {
      assertSegmentMatch("Fog_Bar", "F_B", {
        kind: ts.PatternMatchKind.camelCase,
        isCaseSensitive: true,
      });
    });
    ...
    it("AllLowerPattern3", () => {
      assertSegmentMatch("ABCDEFGH", "bcd", {
        kind: ts.PatternMatchKind.substring,
        isCaseSensitive: false,
      });
    });
  ...
  describe("MultiWordPattern", () => {
    ...
    it("AsteriskSplit", () => {
      assertSegmentMatch("GetKeyWord", "K*W", {
        kind: ts.PatternMatchKind.substring,
        isCaseSensitive: true,
      });
    });
    ...
    it("LowercaseSubstring2", () => {
      assertSegmentMatch("FooAttribute", "a", {
        kind: ts.PatternMatchKind.substring,
        isCaseSensitive: false,
      });
    });
  ...
  describe("DottedPattern", () => {
    it("DottedPattern1", () => {
      assertFullMatch("Foo.Bar.Baz", "Quux", "B.Q", {
        kind: ts.PatternMatchKind.prefix,
        isCaseSensitive: true,
      });
    });
    ...
    it("DottedPattern7", () => {
      assertSegmentMatch("UIElement", "UIElement", {
        kind: ts.PatternMatchKind.exact,
        isCaseSensitive: true,
      });
  ...
  function verifyBreakIntoCharacterSpans(
    original: string,
    ...parts: string[]
  ): void {
  ...
  function verifyBreakIntoWordSpans(
    original: string,
    ...parts: string[]
  ): void {
