  ...
  describe("unittests:: services:: organizeImports", () => {
    describe("Sort imports", () => {
      ...
      function assertSortsBefore(importString1: string, importString2: string) {
        const [
          { moduleSpecifier: moduleSpecifier1 },
          { moduleSpecifier: moduleSpecifier2 },
        ] = parseImports(importString1, importString2);
        assert.equal(
          OrganizeImports.compareModuleSpecifiers(
            moduleSpecifier1,
            moduleSpecifier2
          ),
          Comparison.LessThan
    ...
    describe("Coalesce imports", () => {
      ...
      it("Sort specifiers - case-insensitive", () => {
        ...
        const actualCoalescedImports =
          OrganizeImports.coalesceImports(sortedImports);
        const expectedCoalescedImports = parseImports(
          `import { a as n, B, default as M, y, Z as O } from "lib";`
        );
    ...
    describe("Coalesce exports", () => {
      ...
      it("Sort specifiers - case-insensitive", () => {
        ...
        const actualCoalescedExports =
          OrganizeImports.coalesceExports(sortedExports);
        const expectedCoalescedExports = parseExports(
          `export { a as n, B, default as M, y, Z as O } from "lib";`
        );
      ...
      it("Sort specifiers - type-only", () => {
        const sortedImports = parseImports(
          `import { type z, y, type x, c, type b, a } from "lib";`
        );
        const actualCoalescedImports =
          OrganizeImports.coalesceImports(sortedImports);
        const expectedCoalescedImports = parseImports(
          `import { a, c, y, type b, type x, type z } from "lib";`
        );
        assertListEqual(actualCoalescedImports, expectedCoalescedImports);
      });
...
`,
      ...
      testOrganizeImports(
        "Syntax_Error_Imports_skipDestructiveCodeActions",
        /*skipDestructiveCodeActions*/ true,
        {
          path: "/test.ts",
          content: `
...
`,
      ...
      function testOrganizeExports(
        testName: string,
        testFile: TestFSWithWatch.File,
        ...otherFiles: TestFSWithWatch.File[]
      ) {
        testOrganizeImports(
          `${testName}.exports`,
          /*skipDestructiveCodeActions*/ true,
      ...
      function runBaseline(
        ...
        Harness.Baseline.runBaseline(
          baselinePath,
          ["// ==ORIGINAL==", testContent, "// ==ORGANIZED==", newText].join(
            newLineCharacter
          )
        );
      }
      ...
      function makeLanguageService(...files: TestFSWithWatch.File[]) {
        ...
        projectService.setCompilerOptionsForInferredProjects({
          jsx: files.some((f) => f.path.endsWith("x"))
            ? JsxEmit.React
            : JsxEmit.None,
        });
