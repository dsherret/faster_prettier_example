...
export function Component(x: Config): any;`,
    ...
    it("should be able to create a language service which can respond to deinition requests without throwing", () => {
      ...
      const definitions = languageService.getDefinitionAtPosition(
        "foo.ts",
        160
      ); // 160 is the latter `vueTemplateHtml` position
    ...
    it("getEmitOutput on language service has way to force dts emit", () => {
      const languageService = createLanguageService();
      assert.deepEqual(
        languageService.getEmitOutput("foo.ts", /*emitOnlyDtsFiles*/ true),
        {
          emitSkipped: true,
    ...
    describe("detects program upto date correctly", () => {
      function verifyProgramUptoDate(useProjectVersion: boolean) {
        ...
        files.set("/lib/lib.d.ts", {
          version: "1",
          text: projectSystem.libFile.content,
        });
        const host: LanguageServiceHost = {
          ...
          getProjectVersion: !useProjectVersion
            ? undefined
            : () => projectVersion,
        ...
        files.set("/project/root.ts", {
          version: "2",
          text: `import { foo, bar } from "./other"`,
        });
    ...
    describe("detects program upto date when new file is added to the referenced project", () => {
      function setup(
        ...
        const system = projectSystem.createServerHost([
          config1,
          class1,
          class1Dts,
          config2,
          class2,
          projectSystem.libFile,
        ]);
        const result = getParsedCommandLineOfConfigFile(
          `${tscWatch.projectRoot}/projects/project2/tsconfig.json`,
      ...
      it("detects program upto date when new file is added to the referenced project", () => {
        ...
        system.writeFile(
          `${tscWatch.projectRoot}/projects/project1/class3.d.ts`,
      ...
      it("detects program upto date when new file is added to the referenced project without useSourceOfProjectReferenceRedirect", () => {
        const { ls, system, class1Dts, class2 } = setup(
          /*useSourceOfProjectReferenceRedirect*/ undefined
        );
        const program1 = ls.getProgram()!;
        assert.deepEqual(
