namespace ts.tscWatch {
  ...
  export function createWatchOfConfigFile(
  ...
  export function createWatchOfFilesAndCompilerOptions(
    rootFiles: string[],
    system: WatchedSystem,
  ...
  export function checkOutputErrors(
    ...
    function assertDiagnostic({ diagnostic }: HostOutputDiagnostic) {
      const expected = isDiagnostic(diagnostic)
        ? formatDiagnostic(diagnostic, host)
        : diagnostic;
      assert.equal(
        outputs[index],
        expected,
  ...
  export function checkOutputErrorsInitial(
    host: WatchedSystem,
  ...
  export function checkOutputErrorsIncremental(
    host: WatchedSystem,
    errors: readonly Diagnostic[] | readonly string[],
    disableConsoleClears?: boolean,
  ...
  export function checkOutputErrorsIncrementalWithExit(
    host: WatchedSystem,
    errors: readonly Diagnostic[] | readonly string[],
  ...
  export function getDiagnosticOfFileFrom(
  ...
  export function getDiagnosticOfFile(
    file: SourceFile,
    start: number,
    length: number,
    message: DiagnosticMessage | DiagnosticMessageChain,
  ...
  export function getUnknownDidYouMeanCompilerOption(
    program: Program,
    configFile: File,
    option: string,
    didYouMean: string
  ...
  export function runWatchBaseline({
    ...
    Harness.Baseline.runBaseline(
      `${
        isBuild(commandLineArgs)
          ? isWatch(commandLineArgs)
            ? "tsbuild/watchMode"
            : "tsbuild"
          : isWatch(commandLineArgs)
          ? "tscWatch"
  ...
  function isWatch(commandLineArgs: readonly string[]) {
    return forEach(commandLineArgs, (arg) => {
      ...
      const option = arg
        .slice(arg.charCodeAt(1) === CharacterCodes.minus ? 2 : 1)
        .toLowerCase();
  ...
  export function watchBaseline({
    ...
    const programs = baselinePrograms(
      baseline,
      getPrograms,
      oldPrograms,
      baselineDependencies
