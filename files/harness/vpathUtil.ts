namespace vpath {
  ...
  function validateComponents(
    ...
    const hasExtname =
      hasBasename && extRegExp.test(components[components.length - 1]);
    ...
    if (
      flags & ValidationFlags.RequireTrailingSeparator &&
  ...
  const javaScriptSourceMapExtensions: readonly string[] = [
    ".js.map",
    ".jsx.map",
  ];
  ...
  export function isJavaScriptSourceMap(path: string) {
    return (
      extname(path, javaScriptSourceMapExtensions, /*ignoreCase*/ false)
        .length > 0
    );
  }
  ...
  export function isDefaultLibrary(path: string) {
    return isDeclaration(path) && basename(path).startsWith("lib.");
  }
