...
${indentText}${text}`;
  ...
  function expectedDiagnosticMessageToText([
    message,
    ...args
  ]: ExpectedDiagnosticMessage) {
  ...
  function diagnosticRelatedInformationToText({
    file,
    start,
    length,
    messageText,
...
Expected: ${JSON.stringify(expected, /*replacer*/ undefined, " ")}`,
    ...
    assertErrors(...expectedDiagnostics: ExpectedErrorDiagnostic[]) {
      const actual = this.diagnostics
        .filter((d) => d.kind === DiagnosticKind.Error)
        .map(diagnosticToText);
      const expected = expectedDiagnostics.map(expectedDiagnosticToText);
      assert.deepEqual(
        actual,
        expected,
