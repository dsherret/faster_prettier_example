namespace vfs {
  ...
  export class FileSystem {
    ...
    constructor(ignoreCase: boolean, options: FileSystemOptions = {}) {
      ...
      if ((!cwd || !vpath.isRoot(cwd)) && this._lazy.links) {
        ...
        try {
          for (
            let i = collections.nextResult(iterator);
            i;
            i = collections.nextResult(iterator)
          ) {
            const name = i.value;
    ...
    public mountSync(
      ...
      const {
        ...
        node: existingNode,
        basename,
      } = this._walk(this._resolve(target), /*noFollow*/ true);
    ...
    public writeFileSync(
      ...
      const {
        ...
        node: existingNode,
        basename,
      } = this._walk(this._resolve(path), /*noFollow*/ false);
      if (!parent) throw createIOError("EPERM");
      ...
      if (changedLinks && baseLinks) {
        ...
        baseLinks.forEach((node, basename) => {
          if (!changedLinks.has(basename)) {
            container[basename] = isDirectory(node)
              ? new Rmdir()
              : new Unlink();
            hasChanges = true;
          }
  ...
  export function iteratePatch(
    ...
    // eslint-disable-next-line no-null/no-null
    return patch
      ? Harness.Compiler.iterateOutputs(iteratePatchWorker("", patch))
      : null;
  }
