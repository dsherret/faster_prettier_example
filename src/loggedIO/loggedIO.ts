namespace Playback {
  ...
  export function initWrapper(
    ...[wrapper, underlying]:
      | [PlaybackSystem, ts.System]
      | [PlaybackIO, Harness.IO]
  ): void {
    ...
    function generateTsconfig(
      ...
      return {
        compilerOptions: ts.parseCommandLine(newLog.arguments).options,
        files,
      };
    }
    ...
    wrapper.fileExists = recordReplay(wrapper.fileExists, underlying)(
      (path) =>
        callAndRecord(underlying.fileExists(path), recordLog!.fileExists, {
          path,
        }),
      memoize((path) => {
    ...
    wrapper.resolvePath = recordReplay(wrapper.resolvePath, underlying)(
      (path) =>
        callAndRecord(underlying.resolvePath(path), recordLog!.pathsResolved, {
          path,
        }),
      memoize((path) =>
        findResultByFields(
    ...
    wrapper.readFile = recordReplay(wrapper.readFile, underlying)(
      (path: string) => {
        ...
        const logEntry = {
          path,
          codepage: 0,
          result: { contents: result, codepage: 0 },
        };
    ...
    wrapper.readDirectory = recordReplay(wrapper.readDirectory, underlying)(
      (path, extensions, exclude, include, depth) => {
        const result = (underlying as ts.System).readDirectory(
          path,
          extensions,
          exclude,
          include,
      ...
      (path) => {
        ...
        return ts.flatMap(replayLog!.directoriesRead, (directory) => {
          if (
            ts.normalizeSlashes(directory.path).toLowerCase() === normalizedPath
          ) {
    ...
    wrapper.useCaseSensitiveFileNames = () => {
      ...
      return typeof underlying.useCaseSensitiveFileNames === "function"
        ? underlying.useCaseSensitiveFileNames()
        : underlying.useCaseSensitiveFileNames;
    };
  }
