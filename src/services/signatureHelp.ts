...
namespace ts.SignatureHelp {
  ...
  export function getSignatureHelpItems(
    ...
    if (!candidateInfo) {
      ...
      return isSourceFileJS(sourceFile)
        ? createJSSignatureHelpItems(argumentInfo, program, cancellationToken)
        : undefined;
    }
  ...
  function getImmediatelyContainingArgumentInfo(
    ...
    if (isCallOrNewExpression(parent)) {
      ...
      const argumentIndex = getArgumentIndexForTemplatePiece(
        spanIndex,
        node,
        position,
        sourceFile,
  ...
  function countBinaryExpressionParameters(b: BinaryExpression): number {
    return isBinaryExpression(b.left)
      ? countBinaryExpressionParameters(b.left) + 1
      : 2;
  }
  ...
  function tryGetParameterInfo(
    ...
    const invocation: ContextualInvocation = {
      ...
      node: startingToken,
      symbol: chooseBetterSymbol(nonNullableContextualType.symbol),
    };
    return {
      isTypeParameterList: false,
      invocation,
      ...
      case SyntaxKind.BinaryExpression: {
        ...
        return (
          contextualType && {
            ...
            argumentIndex,
            argumentCount,
            argumentsSpan: createTextSpanFromNode(parent),
          }
        );
      }
      default:
  ...
  function getContainingArgumentInfo(
    ...
    for (
      let n = node;
      !isSourceFile(n) && (isManuallyInvoked || !isBlock(n));
      n = n.parent
    ) {
  ...
  function getEnclosingDeclarationFromInvocation(invocation: Invocation): Node {
    return invocation.kind === InvocationKind.Call
      ? invocation.node
      : invocation.kind === InvocationKind.TypeArgs
      ? invocation.called
      : invocation.node;
  }
  ...
  function getTypeHelpItem(
    ...
    const prefixDisplayParts = [
      ...typeSymbolDisplay,
      punctuationPart(SyntaxKind.LessThanToken),
    ];
    return {
      isVariadic: false,
      prefixDisplayParts,
  ...
  const separatorDisplayParts: SymbolDisplayPart[] = [
    punctuationPart(SyntaxKind.CommaToken),
    spacePart(),
  ];
    ...
    return checker
      ...
      .map((paramList) => {
        ...
        return {
          ...
          suffix: [
            punctuationPart(SyntaxKind.GreaterThanToken),
            ...parameterParts,
          ],
        };
      });
