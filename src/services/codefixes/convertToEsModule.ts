...
namespace ts.codefix {
    ...
    const converted = map(declarationList.declarations, (decl) => {
      ...
      if (initializer) {
        if (isExportsOrModuleExportsOrAlias(sourceFile, initializer)) {
          ...
          isRequireCall(initializer, /*checkArgumentIsStringLiteralLike*/ true)
        ) {
          foundImport = true;
    ...
    if (isExportsOrModuleExportsOrAlias(sourceFile, left)) {
      if (isExportsOrModuleExportsOrAlias(sourceFile, right)) {
        ...
        const replacement = isObjectLiteralExpression(right)
          ...
          : isRequireCall(right, /*checkArgumentIsStringLiteralLike*/ true)
  ...
  function convertReExportAll(
    ...
    const exports = moduleSymbol
      ...
      : (emptyMap as ReadonlyCollection<__String>);
    return exports.has(InternalSymbolName.ExportEquals)
  ...
  function convertExportsDotXEquals_replaceNode(
    ...
    switch (exported.kind) {
      ...
      case SyntaxKind.ClassExpression:
        ...
        return classExpressionToDeclaration(
          name,
          modifiers,
  ...
  function convertSingleImport(
    ...
    switch (name.kind) {
      ...
      case SyntaxKind.Identifier:
        return convertSingleIdentifierImport(
          name,
          moduleSpecifier,
          checker,
          identifiers,
