...
namespace ts.codefix {
  ...
  const fixIdAddDefiniteAssignmentAssertions =
  ...
  registerCodeFix({
    errorCodes,
    getCodeActions: function getCodeActionsForStrictClassInitializationErrors(
      context
    ) {
      ...
      append(
        result,
        getActionForAddMissingDefiniteAssignmentAssertion(context, info)
      );
  ...
  function getActionForAddMissingDefiniteAssignmentAssertion(
    ...
    const changes = textChanges.ChangeTracker.with(context, (t) =>
      addDefiniteAssignmentAssertion(t, context.sourceFile, info.prop)
    );
  ...
  function getActionForAddMissingUndefinedType(
    context: CodeFixContext,
    info: Info
  ): CodeFixAction {
    const changes = textChanges.ChangeTracker.with(context, (t) =>
      addUndefinedType(t, context.sourceFile, info)
    );
    return createCodeFixAction(
      fixName,
      changes,
  ...
  function addUndefinedType(
    ...
    const types = isUnionTypeNode(info.type)
      ? info.type.types.concat(undefinedTypeNode)
  ...
  function getActionForAddMissingInitializer(
    context: CodeFixContext,
    info: Info
  ): CodeFixAction | undefined {
    if (info.isJs) return undefined;
    ...
    const changes = textChanges.ChangeTracker.with(context, (t) =>
      addInitializer(t, context.sourceFile, info.prop, initializer)
    );
    return createCodeFixAction(
      fixName,
      changes,
  ...
  function getDefaultValueFromType(
    ...
    if (type.flags & TypeFlags.BooleanLiteral) {
      ...
      return firstDefined(type.types, (t) =>
        getDefaultValueFromType(checker, t)
      );
    } else if (type.isClass()) {
      const classDeclaration = getClassLikeDeclarationOfSymbol(type.symbol);
      if (
        !classDeclaration ||
        hasSyntacticModifier(classDeclaration, ModifierFlags.Abstract)
      )
        return undefined;

      const constructorDeclaration =
        getFirstConstructorWithBody(classDeclaration);
