...
namespace ts.codefix {
  ...
  const errorCodes = [
    ...
    Diagnostics
      ...
      .code,
    Diagnostics
      .Type_0_must_have_a_Symbol_iterator_method_that_returns_an_iterator.code,
    Diagnostics
  ...
  function getDeclarationSiteFix(
    ...
    if (awaitableInitializers) {
      const initializerChanges = trackChanges((t) => {
        forEach(awaitableInitializers.initializers, ({ expression }) =>
          makeChange(
            t,
            errorCode,
            sourceFile,
  ...
  function symbolReferenceIsAlsoMissingAwait(
    ...
    const errorNode = isPropertyAccessExpression(reference.parent)
      ...
      : isBinaryExpression(reference.parent)
      ? reference.parent
      : reference;
  ...
  function makeChange(
    ...
    if (isBinaryExpression(insertionSite)) {
      for (const side of [insertionSite.left, insertionSite.right]) {
        ...
        const type = checker.getTypeAtLocation(side);
        const newNode = checker.getPromisedTypeOfPromise(type)
          ? factory.createAwaitExpression(side)
          : side;
