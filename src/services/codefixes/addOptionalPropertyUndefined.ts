...
namespace ts.codefix {
  ...
  function getSourceTarget(
    ...
    if (!errorNode) {
      ...
      const name = (n.valueDeclaration as any as SignatureDeclaration)
        .parameters[i].name;
      ...
      return {
        source: isPropertyAssignment(errorNode.parent)
          ? errorNode.parent.initializer
          : errorNode.parent.name,
        target: declaration,
      };
    }
    return undefined;
    ...
    for (const add of toAdd) {
      ...
      if (d && (isPropertySignature(d) || isPropertyDeclaration(d)) && d.type) {
        const t = factory.createUnionTypeNode([
          ...(d.type.kind === SyntaxKind.UnionType
            ? (d.type as UnionTypeNode).types
            : [d.type]),
