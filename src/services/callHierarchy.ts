...
namespace ts.CallHierarchy {
  ...
  function isNamedExpression(node: Node): node is NamedExpression {
    return (
      (isFunctionExpression(node) || isClassExpression(node)) &&
  ...
  export type ConstNamedExpression =
    | (ClassExpression & {
        name: undefined;
        parent: VariableDeclaration & { name: Identifier };
      })
    | (FunctionExpression & {
        name: undefined;
        parent: VariableDeclaration & { name: Identifier };
  ...
  function isValidCallHierarchyDeclaration(
    node: Node
  ): node is CallHierarchyDeclaration {
    return (
      isSourceFile(node) ||
  ...
  function getCallHierarchyItemName(
    ...
    if (isClassStaticBlockDeclaration(node)) {
      ...
      const prefix = symbol
        ? `${typeChecker.symbolToString(symbol, node.parent)} `
        : "";
      return { text: `${prefix}static {}`, pos, end };
    }

    const declName = isConstNamedExpression(node)
      ? node.parent.name
      : Debug.checkDefined(
    ...
    let text = isIdentifier(declName)
      ? idText(declName)
      : isStringOrNumericLiteralLike(declName)
      ? declName.text
      : isComputedPropertyName(declName)
      ? isStringOrNumericLiteralLike(declName.expression)
        ? declName.expression.text
        : undefined
      : undefined;
    if (text === undefined) {
      const typeChecker = program.getTypeChecker();
    ...
    switch (node.kind) {
      ...
      case SyntaxKind.ModuleDeclaration:
        if (
          ...
          isIdentifier(node.parent.parent.name)
        ) {
  ...
  function findImplementation(
    ...
    if (isFunctionDeclaration(node) || isMethodDeclaration(node)) {
      ...
      if (
        ...
        isFunctionLikeDeclaration(symbol.valueDeclaration) &&
  ...
  function findAllInitialDeclarations(
    ...
    if (symbol && symbol.declarations) {
      ...
      indices.sort(
        (a, b) =>
          compareStringsCaseSensitive(keys[a].file, keys[b].file) ||
  ...
  function findImplementationOrAllInitialDeclarations(
    ...
    if (isFunctionLikeDeclaration(node)) {
      return (
        findImplementation(typeChecker, node) ??
  ...
  export function resolveCallHierarchyDeclaration(
    ...
    while (true) {
      ...
      if (isPossibleCallHierarchyDeclaration(location)) {
        ...
        return (
          ancestor &&
          findImplementationOrAllInitialDeclarations(typeChecker, ancestor)
        );
      }
      if (isDeclarationName(location)) {
        ...
        if (
          isVariableDeclaration(location.parent) &&
          location.parent.initializer &&
      ...
      if (
        location.kind === SyntaxKind.StaticKeyword &&
      ...
      // #39453
      if (
        isVariableDeclaration(location) &&
        location.initializer &&
        isConstNamedExpression(location.initializer)
      ) {
  ...
  function convertEntryToCallSite(
    entry: FindAllReferences.Entry
  ): CallSite | undefined {
    if (entry.kind === FindAllReferences.EntryKind.Node) {
      ...
      if (
        isCallOrNewExpressionTarget(
          node,
          /*includeElementAccess*/ true,
          /*skipPastOuterExpressions*/ true
        ) ||
        isTaggedTemplateTag(
          node,
          /*includeElementAccess*/ true,
        ...
        const sourceFile = node.getSourceFile();
        const ancestor =
          findAncestor(node, isValidCallHierarchyDeclaration) || sourceFile;
        return {
  ...
  function convertCallSiteGroupToIncomingCall(
    program: Program,
    entries: readonly CallSite[]
  ) {
  ...
  export function getIncomingCalls(
    ...
    return calls
      ? group(calls, getCallSiteGroupKey, (entries) =>
          convertCallSiteGroupToIncomingCall(program, entries)
        )
      : [];
  }

  function createCallSiteCollector(
    program: Program,
    callSites: CallSite[]
  ): (node: Node | undefined) => void {
    function recordCallSite(
      ...
      const target = isTaggedTemplateExpression(node)
        ? node.tag
    ...
    function collect(node: Node | undefined) {
      ...
      switch (node.kind) {
        ...
        case SyntaxKind.Parameter:
          ...
          collect(
            (node as VariableDeclaration | ParameterDeclaration).initializer
          );
          return;
  ...
  function collectCallSitesOfSourceFile(
    node: SourceFile,
  ...
  function collectCallSitesOfModuleDeclaration(
  ...
  function collectCallSitesOfClassLikeDeclaration(
  ...
  function createCallHierarchyOutgoingCall(
  ...
  function convertCallSiteGroupToOutgoingCall(
    program: Program,
    entries: readonly CallSite[]
  ) {
