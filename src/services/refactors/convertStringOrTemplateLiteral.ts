    ...
    const container = findAncestor(expr.parent, (n) => {
      switch (n.kind) {
        ...
        case SyntaxKind.BinaryExpression:
          return !(
            isBinaryExpression(n.parent) && isNotEqualsOperator(n.parent)
          );
        default:
          return "quit";
      }
  ...
  function treeToArray(current: Expression) {
    const loop = (
      ...
      const {
        nodes,
        operators,
        hasString: leftHasString,
        validOperators: leftOperatorValid,
      } = loop(current.left);
  ...
  function concatConsecutiveString(
    ...
    while (index < nodes.length) {
      ...
      if (isStringLiteralLike(node)) {
        // includes isNoSubstitutionTemplateLiteral(node)
        text += node.text;
