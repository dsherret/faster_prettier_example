...
namespace ts {
  ...
  export function createPatternMatcher(
    ...
    const dotSeparatedSegments = pattern
      .trim()
      .split(".")
      .map((p) => createSegment(p.trim()));
  ...
  function getFullMatch(
    ...
    for (
      let i = dotSeparatedSegments.length - 2,
        j = candidateContainers.length - 1;
      i >= 0;
      i -= 1, j -= 1
    ) {
  ...
  function tryCamelCaseMatch(
    ...
    while (true) {
      ...
      for (
        ;
        currentChunkSpan < chunkCharacterSpans.length;
        currentChunkSpan++
      ) {
