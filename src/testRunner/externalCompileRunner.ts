    ...
    private runTest(directoryName: string) {
      ...
      describe(directoryName, function (this: Mocha.Suite) {
        ...
        it("should build successfully", () => {
          ...
          const npmVersionText = exec("npm", ["--version"], {
            cwd,
            stdio: "pipe",
          })?.trim();
          const npmVersion = npmVersionText
            ? ts.Version.tryParse(npmVersionText.trim())
            : undefined;
...
${stripAbsoluteImportPaths(result.stderr.toString().replace(/\r\n/g, "\n"))}`;
  ...
  export class DockerfileRunner extends ExternalCompileRunnerBase {
      ...
      describe(`${this.kind()} code samples`, function (this: Mocha.Suite) {
        ...
        for (const test of testList) {
          ...
          it(`should build ${directory} successfully`, () => {
            ...
            cls.exec("docker", ["build", "--no-cache", ".", "-t", imageName], {
              cwd,
            }); // --no-cache so the latest version of the repos referenced is always fetched
            const cp: typeof import("child_process") = require("child_process");
            Baseline.runBaseline(
              `${cls.kind()}/${directory}.log`,
              cls.report(
                cp.spawnSync(`docker`, ["run", imageName], {
                  cwd,
                  timeout: cls.timeout,
                  shell: true,
                })
    ...
    private exec(
      ...
      const cp: typeof import("child_process") = require("child_process");
      const stdio = isWorker ? "pipe" : "inherit";
      const res = cp.spawnSync(isWorker ? `${command} 2>&1` : command, args, {
...
${sanitizeDockerfileOutput(result.stderr.toString())}`;
  ...
  function sanitizeUnimportantGulpOutput(result: string): string {
    return result
      .replace(/^.*(\] (Starting)|(Finished)).*$/gm, "") // "gulp" task start/end messages (nondeterministic order)
  ...
  function sanitizeTimestamps(result: string): string {
    return result
      .replace(/\[\d?\d:\d\d:\d\d (A|P)M\]/g, "[XX:XX:XX XM]")
...
${stderr.replace(/\r\n/g, "\n")}`;
  ...
  function isUnexpectedError(cwd: string) {
    return (error: string[]) => {
      ...
      const lines = fs
        .readFileSync(path.join(cwd, errorFile), { encoding: "utf8" })
        .split("\n");
