namespace ts {
  ...
  export function createTestCompilerHost(
    ...
    if (useGetSourceFileByPath) {
      const filesByPath = mapEntries(files, (fileName, file) => [
        toPath(fileName, "", getCanonicalFileName),
        file,
      ]);
  ...
  function mapEqualToCache<T>(
    ...
    const someInLeftHasNoMatch = forEachEntry(left, (leftValue, leftKey) => {
      ...
      return !(valuesAreEqual
        ? valuesAreEqual(leftValue, rightValue)
...
`,
      ...
      {
        name: "b.ts",
        text: SourceText.New(`/// <reference path='c.ts'/>`, "", `var y = 2`),
      },
      { name: "c.ts", text: SourceText.New("", "", `var z = 1;`) },
      {
        name: "types/typerefs/index.d.ts",
        text: SourceText.New("", "", `declare let z: number;`),
      },
    ];
    ...
    it("successful if change affects a single module of a package", () => {
      const files = [
        {
          name: "/a.ts",
          text: SourceText.New("", "import {b} from 'b'", "var a = b;"),
        },
        {
          name: "/node_modules/b/index.d.ts",
          text: SourceText.New("", "export * from './internal';", ""),
        },
        {
          name: "/node_modules/b/internal.d.ts",
          text: SourceText.New("", "", "export const b = 1;"),
        },
        {
...
/// <reference types="typerefs1" />`;
    ...
    it("fails if module kind changes", () => {
      const program1 = newProgram(files, ["a.ts"], {
        target,
    ...
    it("resolution cache follows imports", () => {
      ...
      const files = [
        {
          name: "a.ts",
          text: SourceText.New("", "import {_} from 'b'", "var x = 1"),
        },
      ...
      const updateHost: TestCompilerHost = {
        ...
        getSourceFile(fileName) {
          return fileName === sourceFile.fileName
            ? sourceFile
            : program1.getSourceFile(fileName);
        },
      };
    ...
    it("resolved type directives cache follows type directives", () => {
      const files = [
        ...
        {
          name: "/types/typedefs/index.d.ts",
          text: SourceText.New("", "", "declare var $: number"),
        },
      ];
      const options: CompilerOptions = { target, typeRoots: ["/types"] };
    ...
    it("fetches imports after npm install", () => {
      ...
      const afterNpmInstallProgram = updateProgram(
        initialProgram,
        rootFiles.map((f) => f.name),
        options,
        (f) => {
      ...
      {
        assert.deepEqual(
          afterNpmInstallProgram.host.getTrace(),
          [
            "======== Resolving module 'a' from 'file1.ts'. ========",
            "Explicitly specified module resolution kind: 'NodeJs'.",
            "Loading module 'a' from 'node_modules' folder, target file type 'TypeScript'.",
    ...
    it("can reuse ambient module declarations from non-modified files", () => {
      const files = [
        {
          name: "/a/b/app.ts",
          text: SourceText.New("", "import * as fs from 'fs'", ""),
        },
        {
          name: "/a/b/node.d.ts",
          text: SourceText.New("", "", "declare module 'fs' {}"),
        },
      ];
    ...
    it("can reuse module resolutions from non-modified files", () => {
      const files = [
        ...
        {
          name: "b1.ts",
          text: SourceText.New("", "export class B { x: number; }", ""),
        },
        {
          name: "b2.ts",
          text: SourceText.New("", "export class B { x: number; }", ""),
      ...
      const program6 = updateProgram(
        program5,
        program5.getRootFileNames(),
        options,
        (f) => {
      ...
      const program7 = updateProgram(
        program6,
        program6.getRootFileNames(),
        options,
        (f) => {
          const newSourceText = f[indexOfF1].text.updateImportsAndExports("");
          f[indexOfF1] = { name: "f1.ts", text: newSourceText };
    ...
    describe("redirects", () => {
      ...
      function verifyRedirects(useGetSourceFileByPath: boolean) {
        it("No changes -> redirect not broken", () => {
          ...
          const program2 = updateRedirectProgram(
            program1,
            (files) => {
    ...
    describe("should return true when there is no change in compiler options and", () => {
      ...
      function verifyProgramWithConfigFile(
        ...
        const program = createWatchProgram(
          ...
          .getCurrentProgram()
          .getProgram();
        const { fileNames, options } = parseConfigFileWithSystem(
          configFileName,
          {},
      ...
      it("has paths specified in the options", () => {
        const compilerOptions: CompilerOptions = {
          baseUrl: ".",
          paths: {
            "*": ["packages/mail/data/*", "packages/styles/*", "*"],
          },
        };
        const app: File = {
