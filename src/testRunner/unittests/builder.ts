namespace ts {
  describe("unittests:: builder", () => {
    it("emits dependent files", () => {
      const files: NamedSourceText[] = [
        {
          name: "/a.ts",
          text: SourceText.New("", "import { b } from \"./b\";", ""),
        },
        {
          name: "/b.ts",
          text: SourceText.New(
    ...
    it("if emitting all files, emits the changed file first", () => {
      const files: NamedSourceText[] = [
        {
          name: "/a.ts",
          text: SourceText.New("", "", "namespace A { export const x = 0; }"),
        },
        {
          name: "/b.ts",
          text: SourceText.New("", "", "namespace B { export const x = 0; }"),
        },
      ];
    ...
    it("keeps the file in affected files if cancellation token throws during the operation", () => {
      ...
      const assertChanges = makeAssertChangesWithCancellationToken(
        () => program,
      );
      // No cancellation
  ...
  function makeAssertChanges(
    getProgram: () => Program,
  ): (fileNames: readonly string[]) => void {
    ...
    return (fileNames) => {
      ...
      while (
        builderProgram.emitNextAffectedFile((fileName) => outputFileNames.push(fileName))
      ) {
      }
  ...
  function makeAssertChangesWithCancellationToken(
    ...
    return (fileNames, cancelAfterEmitLength?: number) => {
      ...
      try {
        do {
          ...
          builderProgram.emitNextAffectedFile(
            (fileName) => outputFileNames.push(fileName),
            cancellationToken,
          )
        );
      } catch (e) {
      ...
      assert.equal(
        ...
        fileNames.length > cancelAfterEmitLength!,
      );
      assert.deepEqual(
        outputFileNames,
