namespace ts {
      ...
      files: {
        [root]: {
          ...
          "dev/configs/tests.json": JSON.stringify({
            ...
            exclude: ["../tests/baselines", "../tests/scenarios"],
            include: ["../tests/**/*.ts"],
          }),
  ...
  describe("unittests:: config:: configurationExtension", () => {
    forEach<[string, string, fakes.ParseConfigHost], void>(
      [
        [
          "under a case insensitive host",
          caseInsensitiveBasePath,
          caseInsensitiveHost,
        ],
        [
          "under a case sensitive host",
      ...
      ([testName, basePath, host]) => {
        ...
        function getParseCommandLineJsonSourceFile(entry: string) {
          ...
          return {
            jsonSourceFile,
            parsed: parseJsonSourceFileConfigFileContent(
              jsonSourceFile,
              host,
              basePath,
              {},
        ...
        describe(testName, () => {
          ...
          testFailure(
            "can error when 'extends' is not a string",
            "extends.json",
            [
              {
                code: 5024,
