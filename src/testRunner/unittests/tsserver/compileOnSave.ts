  ...
  describe("unittests:: tsserver:: compileOnSave:: affected list", () => {
    ...
    describe("for configured projects", () => {
      ...
      it("should return results for all projects if not specifying projectFileName", () => {
        const file1: File = {
          path: "/a/b/file1.ts",
          content: "export var t = 10;",
        };
        const file2: File = {
          path: "/a/b/file2.ts",
          content: `import {t} from "./file1"; var t2 = 11;`,
        };
        const file3: File = {
          path: "/a/c/file2.ts",
          content: `import {t} from "../b/file1"; var t3 = 11;`,
        };
        const configFile1: File = {
          path: "/a/b/tsconfig.json",
          content: `{ "compileOnSave": true }`,
        };
        const configFile2: File = {
...
function bar() {
          ...
          assert.deepEqual(affectedFileResponse, [
            {
              fileNames: [
                file.path,
                ...(returnsAllFilesAsAffected
                  ? files.filter((f) => f !== file).map((f) => f.path)
                  : emptyArray),
              ],
              projectFileName: config.path,
              projectUsesOutFile: false,
  ...
  describe("unittests:: tsserver:: compileOnSave:: CompileOnSaveAffectedFileListRequest with and without projectFileName in request", () => {
    ...
    function getSession() {
      ...
      checkProjectActualFiles(project1, [
        libFile.path,
        app1.path,
        core.path,
        app1Config.path,
      ]);
      checkProjectActualFiles(project2, [
        libFile.path,
        app2.path,
        core.path,
        app2Config.path,
    ...
    it("when projectFile is not specified", () => {
      ...
      assert.deepEqual(response, [
        ...
        {
          projectFileName: app2Config.path,
          fileNames: [core.path, app2.path],
          projectUsesOutFile: true,
        },
      ]);
      assert.equal(
