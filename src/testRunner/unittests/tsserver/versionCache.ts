namespace ts {
  ...
  describe(`unittests:: tsserver:: VersionCache TS code`, () => {
    let validateEditAtLineCharIndex: (
      line: number,
      char: number,
      deleteLength: number,
      insertString: string,
...
var q:Point=<Point>p;`;
    ...
    it("handles empty lines array", () => {
      const lineIndex = new server.LineIndex();
      lineIndex.load([]);
      assert.deepEqual(lineIndex.positionToLineOffset(0), {
        line: 1,
        offset: 1,
      });
    });
    ...
    it("handles emptying whole file (GH#44518)", () => {
      ...
      assert.deepEqual(snapshot.positionToLineOffset(0), {
        line: 1,
        offset: 1,
      });
    });
