namespace ts.projectSystem {
  describe("unittests:: tsserver:: events:: ProjectLoadingStart and ProjectLoadingFinish events", () => {
    ...
    function verifyProjectLoadingStartAndFinish(
      createSession: (host: TestServerHost) => {
        session: TestSession;
...
`,
    ...
    describe("when using event handler", () => {
      verifyProjectLoadingStartAndFinish((host) => {
        const { session, events } = createSessionWithEventTracking<
          server.ProjectLoadingStartEvent | server.ProjectLoadingFinishEvent
        >(
          host,
          server.ProjectLoadingStartEvent,
          server.ProjectLoadingFinishEvent,
        ...
        function verifyProjectLoadEvents(
          ...
          const mappedExpected = expected.map((e) => {
            ...
            return {
              ...
              data: { projectName: project.getProjectName(), ...rest },
            };
          });
