  ...
  describe("unittests:: tsserver:: moduleSpecifierCache", () => {
    ...
    it("caches module specifiers within a file", () => {
      ...
      triggerCompletions({
        file: cTs.path,
        line: 1,
        offset: cTs.content.length + 1,
      });
      const mobxCache = moduleSpecifierCache.get(
        cTs.path as Path,
    ...
    it("invalidates the cache when user preferences change", () => {
      ...
      executeSessionRequest<
        protocol.ConfigureRequest,
        protocol.ConfigureResponse
      >(session, protocol.CommandTypes.Configure, { preferences });
      // Nothing changes yet
      assert.ok(getWithPreferences({}));
      ...
      executeSessionRequest<
        protocol.ConfigureRequest,
        protocol.ConfigureResponse
      >(session, protocol.CommandTypes.Configure, {
      ...
      function getWithPreferences(preferences: UserPreferences) {
        return moduleSpecifierCache.get(
          bTs.path as Path,
          aTs.path as Path,
          preferences,
          {},
