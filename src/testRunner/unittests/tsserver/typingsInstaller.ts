namespace ts.projectSystem {
  ...
  describe("unittests:: tsserver:: typingsInstaller:: local module", () => {
    it("should not be picked up", () => {
      ...
      const installer = new (class extends Installer {
        constructor() {
          super(host, {
            typesRegistry: createTypesRegistry("config"),
      ...
      checkProjectActualFiles(configuredProjectAt(service, 0), [
        f1.path,
        f2.path,
        config.path,
      ]);
    ...
    it("configured projects discover from node_modules", () => {
      ...
      const installer = new (class extends Installer {
        constructor() {
          super(host, {
            globalTypingsCacheLocation: "/tmp",
            typesRegistry: createTypesRegistry("jquery", "nested", "commander"),
          });
        }
        installWorker(
          _requestId: number,
      ...
      const installer = new (class extends Installer {
        constructor() {
          super(host, {
            ...
            typesRegistry: createTypesRegistry("commander"),
          });
        }
        installWorker(
          _requestId: number,
    ...
    it("should install typings for unresolved imports", () => {
      ...
      const installer = new (class extends Installer {
        constructor() {
          super(host, {
            globalTypingsCacheLocation: cachePath,
            typesRegistry: createTypesRegistry("node", "commander"),
          });
        }
        installWorker(
          _requestId: number,
    ...
    it("should redo resolution that resolved to '.js' file after typings are installed", () => {
      ...
      const installer = new (class extends Installer {
        constructor() {
          super(host, {
            globalTypingsCacheLocation: cachePath,
            typesRegistry: createTypesRegistry(...typeNames),
          });
        }
        installWorker(
          _requestId: number,
      ...
      checkWatchedFiles(host, [
        ...getConfigFilesToWatch(getDirectoryPath(file.path)),
        "/a/lib/lib.d.ts",
      ]);
    ...
    it("should pick typing names from non-relative unresolved imports", () => {
      ...
      const installer = new (class extends Installer {
        constructor() {
          super(host, {
            globalTypingsCacheLocation: "/tmp",
            typesRegistry: createTypesRegistry("foo"),
          });
        }
        installWorker(
          _requestId: number,
  ...
  describe("unittests:: tsserver:: typingsInstaller:: discover typings", () => {
    ...
    it("should use mappings from safe list", () => {
      ...
      const result = JsTyping.discoverTypings(
        ...
        getDirectoryPath(app.path as Path),
        safeList,
        emptyMap,
        { enable: true },
        emptyArray,
    ...
    it("should return node for core modules", () => {
      ...
      for (const name of JsTyping.nodeCoreModuleList) {
        ...
        const result = JsTyping.discoverTypings(
          ...
          logger.log,
          [f.path],
          getDirectoryPath(f.path as Path),
          emptySafeList,
          cache,
          { enable: true },
          [name, "somename"],
      ...
      const installer = new (class extends Installer {
        ...
        sendResponse(
          ...
          if (response.kind === server.EventEndInstallTypes) {
            assert.deepEqual(response.packagesToInstall, [
              typingsName("commander"),
            ]);
  ...
  describe("unittests:: tsserver:: typingsInstaller:: npm installation command", () => {
    ...
    it("works when the command is too long to install all packages at once", () => {
      ...
      const hasError = TI.installNpmPackages(
        npmPath,
        tsVersion,
        packageNames,
        (command) => {
  ...
  describe("unittests:: tsserver:: typingsInstaller:: recomputing resolutions of unresolved imports", () => {
    ...
    function verifyResolvedModuleOfFooo(project: server.Project) {
      ...
      const foooResolution = project
        .getLanguageService()
        .getProgram()!
        .getSourceFileByPath(appPath)!
        .resolvedModules!.get("fooo", /*mode*/ undefined)!;
