namespace ts.projectSystem {
  function documentSpanFromSubstring({
    file,
    text,
    contextText,
    options,
    contextOptions,
  }: DocumentSpanFromSubstring): DocumentSpan {
    const contextSpan =
  ...
  function makeReferenceEntry({
    isDefinition,
    ...rest
  }: MakeReferenceEntry): ReferenceEntry {
    return {
    ...
    const project = Debug.checkDefined(
      session
        ...
        .getDefaultProjectForFile(
          file.path as server.NormalizedPath,
          /*ensureProject*/ false
        )
    );
    const program = project.getCurrentProgram()!;
    const output = getFileEmitOutput(
      program,
      Debug.checkDefined(program.getSourceFile(file.path)),
  ...
  describe("unittests:: tsserver:: with declaration file maps:: project references", () => {
    ...
    const aDtsMapContent: RawSourceMap = {
      ...
      names: [],
      mappings:
        "AAAA,wBAAgB,GAAG,SAAK;AACxB,MAAM,WAAW,MAAM;CAAG;AAC1B,eAAO,MAAM,SAAS,EAAE,MAAW,CAAC",
    };
    const aDtsMap: File = {
    ...
    function makeSampleProjects(
      ...
      checkNumberOfProjects(
        service,
        addUserTsConfig ? { configuredProjects: 2 } : { inferredProjects: 1 }
      );
      return session;
    }

    function verifyInferredProjectUnchanged(session: TestSession) {
      checkProjectActualFiles(session.getProjectService().inferredProjects[0], [
        userTs.path,
        aDts.path,
        bDts.path,
      ]);
    ...
    function verifyATsConfigProject(session: TestSession) {
      checkProjectActualFiles(
        session.getProjectService().configuredProjects.get(aTsconfig.path)!,
        [aTs.path, aTsconfig.path]
      );
    }
    ...
    function verifyUserTsConfigProject(session: TestSession) {
      checkProjectActualFiles(
        session.getProjectService().configuredProjects.get(userTsconfig.path)!,
        [userTs.path, aTs.path, userTsconfig.path]
      );
    }
    ...
    it("goToDefinition", () => {
      ...
      const response = executeSessionRequest<
        protocol.DefinitionRequest,
        protocol.DefinitionResponse
      >(
        session,
    ...
    it("goToType", () => {
      const session = makeSampleProjects();
      const response = executeSessionRequest<
        protocol.TypeDefinitionRequest,
        protocol.TypeDefinitionResponse
      >(
        session,
        protocol.CommandTypes.TypeDefinition,
        protocolFileLocationFromSubstring(userTs, "instanceA")
      );
      assert.deepEqual(response, [
        protocolFileSpanWithContextFromSubstring({
          file: aTs,
    ...
    it("goToImplementation", () => {
      const session = makeSampleProjects();
      const response = executeSessionRequest<
        protocol.ImplementationRequest,
        protocol.ImplementationResponse
      >(
        session,
    ...
    it("navigateTo", () => {
      const session = makeSampleProjects();
      const response = executeSessionRequest<
        protocol.NavtoRequest,
        protocol.NavtoResponse
      >(session, CommandNames.Navto, { file: userTs.path, searchValue: "fn" });
    ...
    it("navigateToAll -- when neither file nor project is specified", () => {
      const session = makeSampleProjects(
        /*addUserTsConfig*/ true,
        /*keepAllFiles*/ true
      );
      const response = executeSessionRequest<
        protocol.NavtoRequest,
        protocol.NavtoResponse
      >(session, CommandNames.Navto, { file: undefined, searchValue: "fn" });
      assert.deepEqual<readonly protocol.NavtoItem[] | undefined>(response, [
        {
          ...protocolFileSpanFromSubstring({
            file: bTs,
    ...
    it("navigateToAll -- when file is not specified but project is", () => {
      ...
      const response = executeSessionRequest<
        protocol.NavtoRequest,
        protocol.NavtoResponse
      >(session, CommandNames.Navto, {
        projectFileName: bTsconfig.path,
        file: undefined,
        searchValue: "fn",
      });
      assert.deepEqual<readonly protocol.NavtoItem[] | undefined>(response, [
    ...
    const referenceATs = (aTs: File): protocol.ReferencesResponseItem =>
      makeReferenceItem({
        file: aTs,
        isDefinition: true,
        text: "fnA",
    ...
    const referencesUserTs = (
      userTs: File
    ): readonly protocol.ReferencesResponseItem[] => [
      makeReferenceItem({
        file: userTs,
        isDefinition: false,
        text: "fnA",
    ...
    it("findAllReferences", () => {
      ...
      const response = executeSessionRequest<
        protocol.ReferencesRequest,
        protocol.ReferencesResponse
      >(
        session,
    ...
    it("findAllReferences -- starting at definition", () => {
      ...
      >(
        session,
        protocol.CommandTypes.References,
        protocolFileLocationFromSubstring(aTs, "fnA")
      );
      assert.deepEqual<protocol.ReferencesResponseBody | undefined>(response, {
        refs: [referenceATs(aTs), ...referencesUserTs(userTs)],
        symbolName: "fnA",
        symbolStartOffset: protocolLocationFromSubstring(aTs.content, "fnA")
          .offset,
        symbolDisplayString: "function fnA(): void",
      });
    ...
    it("findAllReferencesFull", () => {
      ...
      const responseFull = executeSessionRequest<
        ReferencesFullRequest,
        ReferencesFullResponse
      >(
        session,
      ...
      assert.deepEqual<readonly ReferencedSymbol[]>(responseFull, [
        {
          ...
          references: [
            makeReferenceEntry({
              file: userTs,
              /*isDefinition*/ isDefinition: false,
              text: "fnA",
            }),
            makeReferenceEntry({
              file: aTs,
              /*isDefinition*/ isDefinition: true,
              text: "fnA",
              contextText: "export function fnA() {}",
            }),
          ],
    ...
    it("findAllReferencesFull definition is in mapped file", () => {
      ...
      const session = createSession(
        createServerHost([aTs, aTsconfig, bTs, bTsconfig, aDts, aDtsMap])
      );
      checkDeclarationFiles(aTs, session, [aDtsMap, aDts]);
      openFilesForSession([bTs], session);
      checkNumberOfProjects(session.getProjectService(), {
        configuredProjects: 2,
      }); // configured project of b is alive since a references b
      ...
      >(
        session,
        protocol.CommandTypes.ReferencesFull,
        protocolFileLocationFromSubstring(bTs, "f()")
      );
      ...
      assert.deepEqual<protocol.RenameResponseBody | undefined>(response, {
        info: {
          ...
          kindModifiers: [
            ...
            ScriptElementKindModifier.ambientModifier,
          ].join(","),
    ...
    it("renameLocationsFull", () => {
      const session = makeSampleProjects();
      const response = executeSessionRequest<
        protocol.RenameFullRequest,
        protocol.RenameFullResponse
      >(
        session,
    ...
    it("getEditsForFileRename", () => {
      const session = makeSampleProjects();
      const response = executeSessionRequest<
        protocol.GetEditsForFileRenameRequest,
        protocol.GetEditsForFileRenameResponse
      >(session, protocol.CommandTypes.GetEditsForFileRename, {
      ...
      assert.deepEqual<readonly protocol.FileCodeEdits[]>(response, [
        {
          ...
          textChanges: [
            {
              ...protocolTextSpanFromSubstring(userTs.content, "../a/bin/a"),
              newText: "../a/bin/aNew",
            },
          ],
        },
    ...
    it("getEditsForFileRename when referencing project doesnt include file and its renamed", () => {
      ...
      const response = executeSessionRequest<
        protocol.GetEditsForFileRenameRequest,
        protocol.GetEditsForFileRenameResponse
      >(session, CommandNames.GetEditsForFileRename, {
        oldFilePath: aTs.path,
