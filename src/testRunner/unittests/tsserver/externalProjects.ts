namespace ts.projectSystem {
  describe("unittests:: tsserver:: ExternalProjects", () => {
      ...
      service.openExternalProjects([
        {
          ...
          rootFiles: [toExternalFile(file.path), toExternalFile(app.path)],
          options: {},
        },
      ]);
    });
    ...
    it("can correctly update external project when set of root files has changed", () => {
      ...
      checkNumberOfProjects(projectService, { externalProjects: 1 });
      checkProjectRootFiles(projectService.externalProjects[0], [
        file1.path,
        file2.path,
      ]);
    });
    ...
    it("can update external project when set of root files was not changed", () => {
      ...
      checkProjectActualFiles(projectService.externalProjects[0], [
        file1.path,
        file2.path,
      ]);
    ...
    describe("deleting config file opened from the external project works", () => {
      function verifyDeletingConfigFile(
        ...
        const externalProject: protocol.ExternalProject = {
          projectFileName,
          rootFiles: [
            toExternalFile(site.path),
            toExternalFile(configFile.path),
          ],
    ...
    describe("correctly handling add/remove tsconfig - 2", () => {
      function verifyAddRemoveConfig(
        ...
        checkProjectActualFiles(configuredProjectAt(projectService, 1), [
          dLib.path,
          dTsconfig.path,
        ]);
