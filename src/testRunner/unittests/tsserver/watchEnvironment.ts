namespace ts.projectSystem {
  ...
  describe("unittests:: tsserver:: watchEnvironment:: tsserverProjectSystem watchDirectories implementation", () => {
    function verifyCompletionListWithNewFileInSubFolder(
      ...
      const expectedWatchedFiles = arrayToMap(
        fileNames.slice(1),
        (s) => s,
        () => 1
      );
      const expectedWatchedDirectories = new Map<string, number>();
      const mapOfDirectories =
        tscWatchDirectory === Tsc_WatchDirectory.NonRecursiveWatchDirectory
      ...
      function verifyProjectAndCompletions() {
        const completions = project
          .getLanguageService()
          .getCompletionsAtPosition(index.path, completionPosition, {
            includeExternalModuleExports: false,
            includeInsertTextCompletions: false,
          })!;
        checkArray(
          "Completion Entries",
          completions.entries.map((e) => e.name),
          expectedCompletions
        );
  ...
  describe("unittests:: tsserver:: watchEnvironment:: tsserverProjectSystem Watched recursive directories with windows style file system", () => {
    function verifyWatchedDirectories(
      rootedPath: string,
      useProjectAtRoot: boolean
    ) {
      const root = useProjectAtRoot
        ? rootedPath
        : `${rootedPath}myfolder/allproject/`;
      const configFile: File = {
      ...
      checkProjectActualFiles(
        project,
        files.map((f) => (f === libFile ? winsowsStyleLibFilePath : f.path))
      );
      checkWatchedFiles(
        host,
        mapDefined(files, (f) =>
          f === libFile
            ? winsowsStyleLibFilePath
            : f === file1
            ? undefined
            : f.path
        )
      ...
      checkWatchedDirectories(
        host,
        [root + "project", root + "project/node_modules/@types"].concat(
          useProjectAtRoot ? [] : [root + nodeModulesAtTypes]
        ),
        /*recursive*/ true
      );
    }
  ...
  it(`unittests:: tsserver:: watchEnvironment:: tsserverProjectSystem recursive watch directory implementation does not watch files/directories in node_modules starting with "."`, () => {
    ...
    const fileNames = [index, file1, configFile, libFile].map(
      (file) => file.path
    );
    // All closed files(files other than index), project folder, project/src folder and project/node_modules/@types folder
    const expectedWatchedFiles = arrayToMap(
    ...
    const expectedWatchedDirectories = arrayToMap(
      [
        projectFolder,
        projectSrcFolder,
        `${projectFolder}/${nodeModules}`,
        `${projectFolder}/${nodeModulesAtTypes}`,
      ],
      identity,
      () => 1
    );
  ...
  describe("unittests:: tsserver:: watchEnvironment:: tsserverProjectSystem watching files with network style paths", () => {
    function verifyFilePathStyle(path: string) {
      ...
      checkWatchedFiles(host, [
        libPath,
        `${getDirectoryPath(path)}/tsconfig.json`,
        `${getDirectoryPath(path)}/jsconfig.json`,
      ]);
    }
  ...
  describe("unittests:: tsserver:: watchEnvironment:: handles watch compiler options", () => {
    it("with watchFile option as host configuration", () => {
      ...
      openFilesForSession(
        [{ file: commonFile1, projectRootPath: "/a/b" }],
        session
      );
      checkProjectActualFiles(
    ...
    it("with fallbackPolling option as host configuration", () => {
      ...
      const allFilePaths = filePaths.concat([
        "/a/b",
      ...
      checkWatchedFilesDetailed(
        ...
        1,
        arrayToMap(allFilePaths, toLowerCase, (fileName) => [
          {
            fileName,
            pollingInterval: contains(filePaths, fileName)
              ? PollingInterval.Low
              : PollingInterval.Medium,
          },
        ])
    ...
    it("with watchFile option in configFile", () => {
      ...
      checkWatchedDirectoriesDetailed(
        ...
        1,
        /*recursive*/ false,
        arrayToMap([configFile.path], toLowerCase, (directoryName) => [
          {
            directoryName,
    ...
    describe("excludeDirectories", () => {
      ...
      function setupConfigureHost(
        service: TestProjectService,
        configureHost: boolean | undefined
      ) {
        if (configureHost) {
      ...
      it("inferred project watch options", () => {
        ...
        checkWatchedFilesDetailed(
          ...
          [
            libFile.path,
            `${tscWatch.projectRoot}/tsconfig.json`,
            `${tscWatch.projectRoot}/jsconfig.json`,
            `${tscWatch.projectRoot}/src/tsconfig.json`,
            `${tscWatch.projectRoot}/src/jsconfig.json`,
          ],
          1
        );
        checkWatchedDirectories(host, emptyArray, /*recursive*/ false);
  ...
  describe("unittests:: tsserver:: watchEnvironment:: file names on case insensitive file system", () => {
    function verifyFileNames(projectRoot: string, projectRootPath: string) {
      const keyMapper = (str: string) =>
        str.replace(projectRoot, projectRootPath);
      const file: File = {
      ...
      const expectedWatchFiles = [
        libFile.path,
        `${projectRoot}/tsconfig.json`,
        `${projectRoot}/jsconfig.json`,
      ];
      checkWatchedFilesDetailed(
        host,
        expectedWatchFiles.map(keyMapper),
        1,
        arrayToMap(expectedWatchFiles, keyMapper, (fileName) => [
          {
            fileName,
            pollingInterval: PollingInterval.Low,
          },
      ...
      checkWatchedDirectories(host, [], /*recursive*/ false);
      const expectedWatchedDirectories = [
      ...
      checkWatchedDirectoriesDetailed(
        ...
        1,
        /*recursive*/ true,
        arrayToMap(expectedWatchedDirectories, keyMapper, (directoryName) => [
          {
            directoryName,
            fallbackPollingInterval: PollingInterval.Medium,
            fallbackOptions: {
