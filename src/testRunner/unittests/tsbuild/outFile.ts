  ...
  describe("unittests:: tsbuild:: outFile::", () => {
    ...
    const sources: [Sources, Sources, Sources] = [
      ...
      [
        ...
        ["/src/second/second_part1.ts", "/src/second/second_part2.ts"],
      ],
      ["/src/third/tsconfig.json", ["/src/third/third_part1.ts"]],
    ];
    const relSources = sources.map(([config, sources]) => [
      relName(config),
      sources.map(relName),
    ]) as any as [Sources, Sources, Sources];
    ...
    it("rebuilds completely when command line incremental flag changes between non dts changes", () => {
      ...
      host.assertDiagnosticMessages(
        ...
        [
          ...
          relSources[Project.second][Source.config],
          relSources[Project.second][Source.ts][Part.one],
          relOutputFiles[Project.second][Ext.js],
        ],
        [
