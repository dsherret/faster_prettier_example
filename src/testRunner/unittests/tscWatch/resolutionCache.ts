      ...
      const originalFileExists = host.fileExists;
      {
        ...
        checkOutputErrorsIncremental(host, [
          ...
          getDiagnosticOfFileFromProgram(
            watch.getCurrentProgram().getProgram(),
            root.path,
            newContent.indexOf("var x") + "var ".length,
            "x".length,
...
`,
        ...
        return createWatchedSystem([root, file, libFile], {
          currentDirectory: "/a/b",
        });
      },
      changes: [
...
`
    ...
    describe("ignores files/folder changes in node_modules that start with '.'", () => {
      ...
      verifyIgnore("watch without configFile", [
        "--w",
        `${projectRoot}/test.ts`,
      ]);
      verifyIgnore("watch with configFile", [
        "--w",
        "-p",
      ...
      verifyTscWatch({
        ...
        changes: [
          ...
          },
          {
            ...
            change: (sys) =>
              sys.ensureFileOrFolder({
                path: `${projectRoot}/node_modules/@types/node`,
              }),
            timeouts: runQueuedTimeoutCallbacks,
          },
          {
