namespace ts.tscWatch {
  describe("unittests:: tsc-watch:: projects with references: invoking when references are already built", () => {
      ...
      changes: [
        {
          ...
          change: (sys) => {
            sys.appendFile(
              TestFSWithWatch.getTsBuildProjectFilePath(
                "sample1",
                "logic/index.ts"
              ),
              `function foo() { }`
    ...
    function changeCompilerOpitonsPaths(
      sys: WatchedSystem,
      config: string,
      newPaths: object
    ) {
      ...
      changes: [
        {
          ...
          change: (sys) => {
            ...
            const solutionBuilder = createSolutionBuilder(sys, [
              "tsconfig.b.json",
            ]);
            solutionBuilder.build();
          },
        ...
        },
        {
          ...
          change: (sys) => {
            sys.ensureFileOrFolder({
              path: TestFSWithWatch.getTsBuildProjectFilePath(
                "transitiveReferences",
                "nrefs/a.d.ts"
              ),
              content: sys.readFile(
                TestFSWithWatch.getTsBuildProjectFilePath(
                  "transitiveReferences",
                  "refs/a.d.ts"
                )
...
X;`,
    ...
    verifyTscWatch({
      scenario: "projectsWithReferences",
      subScenario:
        "on transitive references in different folders with no files clause",
      sys: () =>
        createSystemWithSolutionBuild(
          ["c"],
          [
