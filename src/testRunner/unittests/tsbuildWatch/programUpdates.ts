namespace ts.tscWatch {
  ...
  describe("unittests:: tsbuildWatch:: watchMode:: program updates", () => {
    ...
    function transformOutputToOutputFileStamp(
      ...
      return [
        f,
        host.getModifiedTime(f),
        host.writtenFiles.has(host.toFullPath(f)),
      ] as OutputFileStamp;
    }
    ...
    type SubProjectFiles =
      ...
      | [ReadonlyFile, ReadonlyFile, ReadonlyFile, ReadonlyFile];
    ...
    before(() => {
      ...
      testProjectExpectedWatchedFiles = [
        core[0],
        core[1],
        core[2]!,
        ...logic,
      ...
      testProjectExpectedWatchedDirectoriesRecursive = [
        projectPath(SubProject.core),
        projectPath(SubProject.logic),
      ];
    });
    ...
    it("verify building references watches only those projects", () => {
      const system = createTsBuildWatchSystem(allFiles, {
    ...
    describe("validates the changes and watched files", () => {
      ...
      function verifyProjectChanges(
        ...
        verifyTscWatch({
          ...
          commandLineArgs: ["-b", "-w", `${project}/${SubProject.tests}`],
          sys: () =>
            createWatchedSystem(allFilesGetter(), {
              currentDirectory: projectsLocation,
            }),
          changes: [
            changeCore(
...
export class someClass2 { }`),
    ...
    verifyTscWatch({
      ...
      commandLineArgs: ["-b", "-w", `${project}/${SubProject.tests}`],
      sys: () =>
        createWatchedSystem([libFile, ...core, logic[1], ...tests], {
          currentDirectory: projectsLocation,
        }),
      changes: [
        {
...
createSomeObject().message;`,
          ...
          return createWatchedSystem(files, {
            currentDirectory: `${projectsLocation}/${project}`,
          });
        },
        changes: [
          {
...
export function someFn() { }`
    ...
    verifyTscWatch({
      ...
      commandLineArgs: [
        "-b",
        "-w",
        `${project}/${SubProject.core}`,
        "-verbose",
      ],
      sys: () =>
        createWatchedSystem([libFile, ...core], {
          currentDirectory: projectsLocation,
        }),
      changes: [
        noopChange,
