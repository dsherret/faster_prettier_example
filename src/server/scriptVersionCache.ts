...
namespace ts.server {
  ...
  class EditWalker implements LineIndexWalker {
    ...
    insertLines(
      ...
      if (lines.length > 0) {
        ...
        const insertionNode = this.startPath[
          this.startPath.length - 2
        ] as LineNode;
  ...
  export class LineIndex {
    ...
    private positionToColumnAndLineText(position: number): {
  ...
  class LineNode implements LineCollection {
    ...
    charOffsetToLineInfo(
      ...
      if (lineCount === 0) {
        // it's empty! (and lineNumberToInfo expects a one-based line)
      ...
      return {
        oneBasedLine: lineCount,
        zeroBasedColumn: leaf.charCount(),
        lineText: undefined,
      };
    }
