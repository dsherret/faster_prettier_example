...
namespace ts {
  ...
  export function canProduceDiagnostics(
    node: Node,
  ): node is DeclarationDiagnosticProducing {
    return (
  ...
  export function createGetSymbolAccessibilityDiagnosticForNodeName(
    ...
    function getAccessorNameVisibilityError(
      symbolAccessibilityResult: SymbolAccessibilityResult,
    ) {
    ...
    function getAccessorNameVisibilityDiagnosticMessage(
      symbolAccessibilityResult: SymbolAccessibilityResult,
    ) {
      if (isStatic(node)) {
    ...
    function getMethodNameVisibilityDiagnosticMessage(
      symbolAccessibilityResult: SymbolAccessibilityResult,
    ) {
      if (isStatic(node)) {
        return symbolAccessibilityResult.errorModuleName
  ...
  export function createGetSymbolAccessibilityDiagnosticForNode(
    ...
    if (
      ...
      return Debug.assertNever(
        ...
        `Attempted to set a declaration diagnostic context for unhandled node kind: ${
          (ts as any).SyntaxKind[(node as any).kind]
        }`,
      );
    }
    ...
    function getParameterDeclarationTypeVisibilityDiagnosticMessage(
      ...
      switch (node.parent.kind) {
        ...
        default:
          return Debug.fail(
            `Unknown parent for parameter: ${(ts as any).SyntaxKind[node.parent.kind]}`,
          );
      }
    }
    ...
    function getHeritageClauseVisibilityError(): SymbolAccessibilityDiagnostic {
      ...
      if (isClassDeclaration(node.parent.parent)) {
        ...
        diagnosticMessage = isHeritageClause(node.parent)
    ...
    function getTypeAliasDeclarationVisibilityError(
      ...
      return {
        ...
        errorNode: isJSDocTypeAlias(node)
          ? Debug.checkDefined(node.typeExpression)
          : (node as TypeAliasDeclaration).type,
        typeName: isJSDocTypeAlias(node)
          ? getNameOfDeclaration(node)
          : (node as TypeAliasDeclaration).name,
      };
    }
  }
}
