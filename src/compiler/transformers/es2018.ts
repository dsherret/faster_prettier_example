...
namespace ts {
  ...
  export function transformES2018(context: TransformationContext) {
      ...
      if (node.transformFlags & TransformFlags.ContainsObjectRestOrSpread) {
        ...
        if (objects.length > 1) {
          for (let i = 1; i < objects.length; i++) {
            expression = emitHelpers().createAssignHelper([
              expression,
              objects[i],
            ]);
          }
    ...
    function convertForOfStatementHead(
      ...
      let statementsLocation: TextRange | undefined;
      const statements: Statement[] = [
        visitNode(binding, visitor, isStatement),
      ];
    ...
    function collectParametersWithPrecedingObjectRestOrSpread(
      ...
      for (const parameter of node.parameters) {
        if (parameters) {
          ...
          parameter.transformFlags & TransformFlags.ContainsObjectRestOrSpread
        ) {
          parameters = new Set();
        }
      }
    ...
    function transformAsyncGeneratorFunctionBody(
      ...
      if (emitSuperHelpers && hasSuperElementAccess) {
        if (
          ...
          resolver.getNodeCheckFlags(node) & NodeCheckFlags.AsyncMethodWithSuper
        ) {
