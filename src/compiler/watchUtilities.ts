...
namespace ts {
  ...
  export function isIgnoredFileFromWildCardWatching({
    ...
    if (fileExtensionIs(fileOrDirectoryPath, Extension.Dts)) {
      ...
      !fileExtensionIsOneOf(fileOrDirectoryPath, supportedJSExtensionsFlat)
    ) {
      return false;
    }
    ...
    const realProgram = isArray(program)
      ? undefined
  ...
  export function getWatchFactory<X, Y = undefined>(
    ...
    function getWatchInfo<T>(
      ...
      return `WatchInfo: ${file} ${flags} ${JSON.stringify(options)} ${
        getDetailWatchInfo
          ? getDetailWatchInfo(detailInfo1, detailInfo2)
