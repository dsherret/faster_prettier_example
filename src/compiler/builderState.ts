...
namespace ts {
  ...
  export namespace BuilderState {
    ...
    function deleteFromMultimap<K, V>(
      map: ESMap<K, Set<V>>,
      k: K,
      v: V,
      removeEmpty = true,
    ...
    export function updateShapeSignature(
      ...
      if (!sourceFile.isDeclarationFile && !useFileVersionAsSignature) {
        ...
        if (firstDts) {
          Debug.assert(
            fileExtensionIsOneOf(firstDts.name, [
              Extension.Dts,
              Extension.Dmts,
              Extension.Dcts,
            ]),
      ...
      if (latestSignature === undefined) {
        ...
        if (exportedModulesMapCache && latestSignature !== prevSignature) {
          ...
          const references = state.referencedMap
            ? state.referencedMap.getValues(sourceFile.resolvedPath)
            : undefined;
          if (references) {
    ...
    function updateExportedModules(
      ...
      exportedModulesFromDeclarationEmit.forEach((symbol) =>
        addExportedModule(getReferencedFilesFromImportedModuleSymbol(symbol))
      );
      if (exportedModules) {
    ...
    export function updateExportedFilesMapFromCache(
      ...
      if (exportedModulesMapCache) {
        ...
        exportedModulesMapCache
          .deletedKeys()
          ?.forEach((path) => state.exportedModulesMap!.deleteKey(path));
